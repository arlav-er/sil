...
public class CMSetDefinitivaGraduatoria extends AbstractSimpleModule {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger
			.getLogger(CMSetDefinitivaGraduatoria.class.getName());

	/*
	 * public void service(SourceBean request, SourceBean response) throws Exception { // per ora faccio l'aggiornamento
	 * a definitiva solamente della // graduatoria per il profilo che sto visualizzando doUpdate(request,response); }
	 */
	...
	public void service(SourceBean request, SourceBean response) throws EMFInternalError {
		...
		try {
			...
			setGraduatoriaDefinitiva(request, response);
	...
	public void setGraduatoriaDefinitiva(SourceBean request, SourceBean response) {
		...
		try {
			...
			statementSB = (SourceBean) getConfig().getAttribute("CM_UPDATE_SET_DEFINITIVA_GRD");
			...
			if (!codiceRit.equals("0")) {
				...
				case -1: // errore generico sql
					...
					msg = "errore: sqlCode=" + errCode;

					break;
				default:
					msgCode = MessageCodes.General.OPERATION_FAIL;
					msg = "errore di ritorno non ammesso. SqlCode=" + errCode;
