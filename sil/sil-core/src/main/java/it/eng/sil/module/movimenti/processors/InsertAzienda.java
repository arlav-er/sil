 ...
 */
public class InsertAzienda implements RecordProcessor {
	 ...
	 */
	public InsertAzienda(String name, TransactionQueryExecutor transexec, BigDecimal user, String configFileName,
			SourceBean sbGenerale) throws SAXException, FileNotFoundException, IOException,
	...
	public SourceBean processRecord(Map record) throws SourceBeanException {
		...
		if (!(prgAz != null && prgUnita != null)) {
			...
			try {
				...
				return ProcessorsUtils.createResponse(name, classname,
						new Integer(MessageCodes.ImportMov.ERR_INSERT_DATA), "Impossibile cercare l'azienda", warnings,
						nested);
			}
			...
			if (prgAzList.size() == 0) {
				...
				// ci pensa il processor insert ad aggiungere il prgAzienda nel
				// record
				...
				if (!strNumAlboInterinali.equals("")) {
					...
					/*
					 * Se il contratto del rapporto di lavoro è H.01.00 allora l'anagrafica azienda viene creata con
					 * tipologia AGR = Agricola
					 */
					if (codTipoAss.equals("H.01.00")) {
						record.put("CODAZTIPOAZIENDA", "AGR");
					} else {
						/*
						 * Se il contratto del rapporto di lavoro è I.01.00 oppure I.02.00 allora l'anagrafica
						 * azienda viene creata con tipologia PA = Pubblica amministrazione
						 */
				...
				warnings.add(new Warning(MessageCodes.ImportMov.AGGIUNTA_AZ,
						"L'azienda '" + ragSocAz + "' con codice fiscale: " + codfisc + " è stata aggiunta al DB."));
				gestisciSede = true;
				...
				// Aggiorno i dati dell'azienda se sono in validazione massiva
					...
					if (updateAzResult != null) {
						...
						if (ProcessorsUtils.isError(updateAzResult)) {
							return ProcessorsUtils.createResponse(name, classname,
									...
									"Impossibile aggiornare i dati dell'azienda '" + ragSocAz + "' nel DB.", warnings,
									nested);
						}
					}
				}
				...
				return ProcessorsUtils
						.createResponse(name, classname, new Integer(MessageCodes.ImportMov.ERR_INSERT_DATA),
			...
			ArrayList prgUnitaProdList = null;
			try {
				prgUnitaProdList = findUnita((BigDecimal) record.get("PRGAZIENDA"), codComUnitaAz);
			} catch (Exception e) {
				return ProcessorsUtils.createResponse(name, classname,
			...
			if (prgUnitaProdList.size() == 0) {
				...
				try {
					prgUnitaProdCheckedList = checkIndirizzo((BigDecimal) record.get("PRGAZIENDA"), prgUnitaProdList,
							indirUnitaAz);
				...
				if (prgUnitaProdCheckedList.size() == 0) {
					...
					// Aggiorno i dati dell'unita produttiva se sono in validazione massiva
					if (validazioneMassiva) {
			...
			if (gestisciSede) {
				...
				ArrayList prgSedeLegalelist = null;
				try {
					prgSedeLegalelist = findUnita((BigDecimal) record.get("PRGAZIENDA"),
							(String) record.get("CODUALCOM"));
				} catch (Exception e) {
				...
				if (okSede && prgSedeLegalelist.size() == 0) {
					...
					try {
						prgSedeLegaleCheckedList = checkIndirizzo((BigDecimal) record.get("PRGAZIENDA"),
								prgSedeLegalelist, indirSedeLeg);
					} catch (Exception e) {
					...
					if (okSede && prgSedeLegaleCheckedList.size() == 0) {
						...
						// Aggiorno i dati della sede legale se sono in validazione massiva
						if (validazioneMassiva) {
		...
		if ((notAssPropria) || (distacco && notDistaAzEstera)
			...
			if (!(prgAzUtil != null && prgUnitaUtil != null)) {
				...
				// Controllo che i parametri necessari per le query di ricerca
				// ci siano
				...
				if (prgAzUtilList.size() == 0) {
					// l'azienda non è presente, la inserisco;
					// ci pensa il processor insert ad aggiungere il prgAzienda
					// nel record
					...
					// Aggiorno i dati dell'azienda utilizzatrice se sono in validazione massiva
					if (validazioneMassiva) {
				...
				ArrayList prgUnitaUtilList = null;
				try {
					prgUnitaUtilList = findUnita((BigDecimal) record.get("PRGAZIENDAUTIL"), codComUnitaAzUtil);
				} catch (Exception e) {
					return ProcessorsUtils.createResponse(name, classname,
							new Integer(MessageCodes.ImportMov.ERR_INSERT_DATA),
				...
				// Inserisco l'unita produttiva dell'azienda utilizzatrice se
				// non esiste
				if (prgUnitaUtilList.size() == 0) {
					// L'unita non esiste, la aggiungo
					SourceBean unitaProdUtilResult = null;
					try {
						unitaProdUtilResult = insertUnita((BigDecimal) record.get("PRGAZIENDAUTIL"), insertUnitaUtil,
								"PRGUNITAUTIL", record);
					} catch (Exception e) {
						return ProcessorsUtils.createResponse(name, classname,
								new Integer(MessageCodes.ImportMov.ERR_INSERT_DATA),
								"Impossibile inserire l'unita produttiva dell'azienda utilizzatrice", warnings, nested);
					}
					...
					// Segnalo che ho aggiunto l'unita produttiva dell'azienda
					// utilizzatrice con una warning
					warnings.add(new Warning(MessageCodes.ImportMov.AGGIUNTA_UAZ,
							"L'unita aziendale dell'azienda utilizzatrice '" + ragSocAzUtil + "' con codice fiscale: "
									+ codfiscUtil + " e indirizzo " + indirUnitaAzUtil + " è stata aggiunta al DB."));
				} else {
					...
					try {
						prgUnitaUtilCheckedList = checkIndirizzo((BigDecimal) record.get("PRGAZIENDAUTIL"),
								prgUnitaUtilList, indirUnitaAzUtil);
					} catch (Exception e) {
						return ProcessorsUtils.createResponse(name, classname,
								new Integer(MessageCodes.ImportMov.ERR_INSERT_DATA),
								"Impossibile filtrare l'unita utilizzatrice in base all'indirizzo", warnings, nested);
					}
					...
					if (prgUnitaUtilCheckedList.size() == 0) {
						...
						// ho individuato univocamente l'unita utilizzatrice, la
						// riporto nel record
						record.put("PRGUNITAUTIL", prgUnitaUtilCheckedList.get(0));

						// Aggiorno i dati dell'unita produttiva dell'azienda utilizzatrice se sono in validazione
						// massiva
						if (validazioneMassiva) {
	 ...
	 */
	private ArrayList findUnita(BigDecimal prgAzienda, String codiceComune) throws SQLException {
	 ...
	 */
	private ArrayList checkIndirizzo(BigDecimal prgAzienda, ArrayList prgUnitaList, String indirizzo)
			throws SQLException {

		if (prgAzienda == null || prgUnitaList == null || prgUnitaList.size() == 0 || indirizzo == null) {
			return new ArrayList();
		}

		// Controllo se l'indirizzo coincide con la query
		StringBuffer checkInd = new StringBuffer(
				"SELECT prgUnita FROM AN_UNITA_AZIENDA WHERE prgAzienda = " + prgAzienda + " AND prgUnita IN ( ");
		for (int i = 0; i < prgUnitaList.size(); i++) {
			checkInd.append((i > 0 ? ", " : ""));
			checkInd.append(" " + prgUnitaList.get(i).toString() + " ");
		}
		indirizzo = StringUtils.replace(indirizzo, "'", "''").toUpperCase();
		checkInd.append(" ) AND UPPER(STRINDIRIZZO) = '" + indirizzo + "'");
		String query = checkInd.toString();
		// Provo a cercare l'unita aziendale
		ArrayList unitaList = getProgressivo(query);
		return unitaList;
	}
