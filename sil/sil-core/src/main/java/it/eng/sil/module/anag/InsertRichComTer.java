...
public class InsertRichComTer extends AbstractSimpleModule {
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		try {
			...
			BigDecimal PRGRICHCOMPTERR = doNextVal(request, response);

			if (PRGRICHCOMPTERR == null) {
				throw new Exception("Impossibile leggere S_CM_RICH_COMP_TERR.NEXTVAL");
			}

			request.delAttribute("PRGRICHCOMPTERR");
			request.delAttribute("strChiaveTabella");
			request.setAttribute("PRGRICHCOMPTERR", PRGRICHCOMPTERR.toString());
			request.setAttribute("strChiaveTabella", PRGRICHCOMPTERR.toString());

			this.setSectionQueryInsert("QUERY_INSERT_RICH_COMTER");
			ret = doInsert(request, response);

			if (!ret) {
				throw new Exception("impossibile inserire in CM_RICH_PRGRICHCOMPTERR in transazione");
			} else {
			...
			response.delAttribute("PRGRICHCOMPTERR");
			response.setAttribute("PRGRICHCOMPTERR", PRGRICHCOMPTERR);
