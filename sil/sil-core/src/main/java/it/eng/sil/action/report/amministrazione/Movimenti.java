...
public class Movimenti extends AbstractSimpleReport {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(Movimenti.class.getName());

	private static final String thisClassName = StringUtils.getClassName(Movimenti.class);
	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			if (tipoStampa.equalsIgnoreCase("LISTA")) {
				doSelMovimentiRicerca();
			} else {
				if (tipoStampa.equalsIgnoreCase("VALIDATI")) {
					prgValidazioneMassiva = (String) request.getAttribute("prgValidazioneMassiva");
					doSelMovimentiValidati();
				} else {
			...
			if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
				...
				if (tipoFile != null) {
					setStrNomeDoc("ReportMovimenti." + tipoFile);
				} else {
					setStrNomeDoc("ReportMovimenti.pdf");
				}
				setStrDescrizione("Stampa movimenti amministrativi");
				setReportPath("Amministrazione/Movimenti_CC.rpt");
	...
	/**
	 * Recupera i progressivi dei movimenti validati e li mette in vettPrgMovimenti.
	 */
	private void doSelMovimentiValidati() throws Exception {

		SourceBean sb = null;
		try {
			openConnection();
			sb = doSelect("GET_MOVIMENTI_VALIDATI");
			List righe = sb.getAttributeAsVector("ROW");
			BigDecimal prgMov = null;
			for (int i = 0; i < righe.size(); i++) {
				prgMov = (BigDecimal) ((SourceBean) righe.get(i)).getAttribute("PRGMOV");
