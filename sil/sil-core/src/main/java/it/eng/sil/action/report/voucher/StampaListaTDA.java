...
public class StampaListaTDA extends AbstractSimpleReport {
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(StampaListaTDA.class.getName());
	...
	public void service(SourceBean request, SourceBean response) {
		...
		if (tipoFile != null)
			setStrNomeDoc("ListaVouchers." + tipoFile);
		else
			setStrNomeDoc("ListaVouchers.pdf");

		setStrDescrizione("Lista Vouchers - Titoli di Acquisto");
		setReportPath("Voucher/ListaVouchers.rpt");
		...
		try {
			com.inet.report.Engine eng = executeQueries(request, response);
	...
	com.inet.report.Engine executeQueries(SourceBean request, SourceBean response) throws Exception {
		String tipoFile = (String) request.getAttribute("tipoFile");
		SourceBean listaVouchers = doDynamicSelect(request, response);

		Vector vVouchers = listaVouchers.getAttributeAsVector("ROW");

		// ApiConservazioneLSU report = new ApiConservazioneLSU((SourceBean) getConfig());
		ApiStampaTDA report = new ApiStampaTDA((SourceBean) getConfig());
		...
		report.setElencoVouchers(vVouchers);
