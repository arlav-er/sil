...
public class StampaLavoratoriMov extends AbstractSimpleReport {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(StampaLavoratoriMov.class.getName());
	...
	public void service(SourceBean request, SourceBean response) {
		...
		if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
			...
			if (tipoFile != null)
				setStrNomeDoc("StampaLavoratoriMov." + tipoFile);
			else
				setStrNomeDoc("StampaLavoratoriMov.pdf");
			setStrDescrizione("Stampa Lavoratori Mov");
	...
	Engine executeQueryAdesioni(SourceBean request, SourceBean response) throws Exception {
		...
		String codMonoTempo = Utils.notNull(request.getAttribute("codMonoTempo"));
		String codCMTipoIscr = Utils.notNull(request.getAttribute("codCMTipoIscr"));

		String dataMovDa = Utils.notNull(request.getAttribute("dataMovDa"));
		String dataMovA = Utils.notNull(request.getAttribute("dataMovA"));
		...
		String cdnLavoratorePrec = "";

		for (int i = 0; i < sizeVec; i++) {
			SourceBean richiesta = (SourceBean) vecAdesioniChiamata.get(i);

			String cdnLavoratore = "" + richiesta.getAttribute("CDNLAVORATORE");

			if (!cdnLavoratore.equals(cdnLavoratorePrec)) {
				finale.addElement(richiesta);
			}

			cdnLavoratorePrec = cdnLavoratore;
		}

		ApiStampaLavoratoriMov report = new ApiStampaLavoratoriMov((SourceBean) getConfig());
		...
		report.setDataMovDa(dataMovDa);
		report.setDataMovA(dataMovA);
