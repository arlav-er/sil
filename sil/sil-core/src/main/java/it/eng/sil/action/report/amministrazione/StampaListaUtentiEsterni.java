...
public class StampaListaUtentiEsterni extends AbstractSimpleReport {
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger
			.getLogger(StampaListaUtentiEsterni.class.getName());
	...
	public void service(SourceBean request, SourceBean response) {
		...
		this.dataCPM = (StringUtils.getAttributeStrNotNull(request, "dataIn")).trim();
		// this.dataDomandaA = (StringUtils.getAttributeStrNotNull(request, "dataInA")).trim();
		...
		if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
			...
			if (tipoFile != null)
				setStrNomeDoc("ListaMobUtentiEsterni." + tipoFile);
			else
				setStrNomeDoc("ListaMobUtentiEsterni.pdf");
			setStrDescrizione("Lista mobilità utenti esterni");
			setReportPath("Amministrazione/ListaMobUtentiEsterni_CC.rpt");
	...
	com.inet.report.Engine executeQueries(SourceBean request, SourceBean response) throws Exception {
		...
		ApiMobilitaUtentiEsterni report = new ApiMobilitaUtentiEsterni((SourceBean) getConfig());
