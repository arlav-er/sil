...
public class MobilitaRicercaCsv implements IDynamicStatementProvider {

	private String className = this.getClass().getName();

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(MobilitaRicercaCsv.class.getName());

	private static final String SELECT_SQL_BASE = " NVL(LAV.STRCODICEFISCALE,' ') CF,"
			+ " NVL(LAV.STRCOGNOME,' ') COGNOME," + " NVL(LAV.STRNOME,' ') NOME"
	// " MOB.PRGMOBILITAISCR," +
	// " MOB.CDNLAVORATORE," +
	// " case " +
	...
	// " end storicizzato, " +
	// " MOB.CODTIPOMOB, " +
	// " STATO_RICH.STRDESCRIZIONE stato, " +
	...
	public String getStatement(RequestContainer requestContainer, SourceBean config) {
		...
		dsu.addSelect(SELECT_SQL_BASE); // AAA SPOSTATO QUI

		// INIT-PARTE-TEMP
		if (Sottosistema.MO.isOff()) {
		} else {
			// END-PARTE-TEMP
			dsu.addSelect("NVL(AN_AZIENDA.STRRAGIONESOCIALE,' ') Rag___Soc___Azienda");
			dsu.addSelect("NVL(AN_UNITA_AZIENDA.STRINDIRIZZO,' ') Ind___Azienda");
			// dsu.addSelect("DE_COMUNE.STRDENOMINAZIONE AS COMUNE");
			// dsu.addSelect("(LAV.STRCODICEFISCALE || '\n' || LAV.STRCOGNOME || '\n' ||LAV.STRNOME) AS LAVORATORE ");
			// INIT-PARTE-TEMP
		}

		dsu.addSelect("  NVL(to_char(MOB.DATINIZIO,'DD/MM/YYYY'),' ') as Data_Inizio");
		dsu.addSelect("  NVL(to_char(MOB.DATFINE,'DD/MM/YYYY'),' ') as Data_Fine");
		dsu.addSelect("  NVL(STATO_RICH.STRDESCRIZIONE,' ') as Stato_della_Richiesta");

		dsu.addSelect(" NVL(case " + "    when ma.codlistespec is not null "
				+ "    then (DE_MB_TIPO.STRDESCRIZIONE || ' (Min: ' || mn.des_listespec|| ')' ) "
				+ "    else DE_MB_TIPO.STRDESCRIZIONE || ' (Min: nessuna corrispondenza)' "
				+ " end,' ') as Tipo_Lista");

		dsu.addSelect("  NVL(to_char(MOB.DATMAXDIFF ,'DD/MM/YYYY'),' ') as Data_Diff");
		dsu.addSelect("  NVL(to_char(MOB.DATCRT ,'DD/MM/YYYY'),' ') as Data_CRT");

		if (Sottosistema.MO.isOff()) {
		} else {
			dsu.addSelect("NVL(DECODE(MOB.CODMONOPROV,'P','Da Prolabor','I','Ins. manuale','F','Da File',"
					+ "'A','Da Prolabor/Agg. manuale','M','Ins./Agg. manuale','C','Da File/Agg. manuale'),' ') AS Prov__");
		}
		// END-PARTE-TEMP
		...
		// dsu.addSelect(SELECT_SQL_BASE); // AAA SPOSTATO IN ALTO
		// INIT-PARTE-TEMP
		// if (Sottosistema.MO.isOff()) {
		// } else {
		// END-PARTE-TEMP
		...
		// dsu.addSelect("DE_COMUNE.STRDENOMINAZIONE AS COMUNE");
		// dsu
		// .addSelect("DECODE(MOB.CODMONOPROV,'P','Da Prolabor','I','Ins.\nmanuale','F','Da File',"
		// + "'A','Da \nProlabor/\nAgg.\nmanuale','M','Ins./\nAgg.\n manuale','C','Da File/\nAgg.\nmanuale') AS
		// PROVENIENZA");
		// dsu.addSelect("(LAV.STRCODICEFISCALE || '\n' || LAV.STRCOGNOME || '\n' ||LAV.STRNOME) AS LAVORATORE ");
		// INIT-PARTE-TEMP
