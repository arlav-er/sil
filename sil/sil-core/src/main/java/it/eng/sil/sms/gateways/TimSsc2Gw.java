 ...
 */
public class TimSsc2Gw extends BaseGateway {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(TimSsc2Gw.class.getName());
	...
	public void send() throws SmsException {
		...
		while (iter.hasNext()) {
			...
			try {
				...
				message.setStringProperty("gateway", "TIM-SSC2");
				...
				message.setStringProperty("baseUrl", baseUrl);
				message.setStringProperty("token", token);
				message.setStringProperty("alias", alias);

				if (proxyHost != null && !"".equals(proxyHost)) {
					message.setStringProperty("proxyHost", proxyHost);
					message.setStringProperty("proxyPort", proxyPort);
					message.setStringProperty("proxyPassword", proxyPassword);
					message.setStringProperty("proxyUsername", proxyUsername);
				}
