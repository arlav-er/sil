 ...
 * 
 */
public class CMDeleteRichiestaAdesione extends AbstractSimpleModule {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger
			.getLogger(CMDeleteRichiestaAdesione.class.getName());
	...
	public void service(SourceBean request, SourceBean response) throws EMFInternalError {
		...
		try {
			...
			deleteAdesioneCM(request, response);

			transExec.commitTransaction();
			ror.reportSuccess(idSuccess);
		} catch (Exception ex) {
			it.eng.sil.util.TraceWrapper.debug(_logger,
					"CMDeleteRichiestaAdesione::service(): Impossibile cancellare l'adesione!", ex);
	...
	public void deleteAdesioneCM(SourceBean request, SourceBean response) {
		...
		try {
			...
			statementSB = (SourceBean) getConfig().getAttribute("CM_DELETE_ADESIONE");
