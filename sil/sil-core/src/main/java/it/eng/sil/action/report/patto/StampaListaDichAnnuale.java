...
public class StampaListaDichAnnuale extends AbstractSimpleReport {
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(StampaListaDichAnnuale.class.getName());
	...
	public void service(SourceBean request, SourceBean response) {
		...
		if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
			...
			if (tipoFile != null)
				setStrNomeDoc("ListaDichAnnuale." + tipoFile);
			else
				setStrNomeDoc("ListaDichAnnuale.pdf");

			setStrDescrizione("Lista Lavoratori Dich.Annuale");
			setReportPath("patto/ListaDichAnnuale_CC.rpt");
	...
	com.inet.report.Engine executeQueries(SourceBean request, SourceBean response) throws Exception {
		...
		ApidDichAnnuale report = new ApidDichAnnuale((SourceBean) getConfig());
