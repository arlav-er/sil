...
public class AppuntamentiModule extends AbstractBasicListModule {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(AppuntamentiModule.class.getName());

	public AppuntamentiModule() {
	}
	...
	public ListIFace getList(SourceBean request, SourceBean response) {
		...
		if (MOD.equals("0") || MOD.equals("")) {
			moduleName = "MAPPUNTAMENTI";
		}
		if (MOD.equals("2")) {
			moduleName = "MAPPUNTAMENTIRICERCA";
		}
		if (MOD.equals("3")) {
			moduleName = "MCONTATTIRICERCA";
		}
		String umbria = StringUtils.getAttributeStrNotNull(request, "configLabelServizio");
		System.out.println(umbria);
		...
		try {
			...
			if ((rowsSourceBean == null) || (rowsVector.size() == 0)) {
				...
				SourceBean row = (SourceBean) rowsVector.get(0);
			...
			for (int i = 0; i < columnNameBeans.size(); i++) {
				...
				String nomeColonna = (String) ((SourceBean) columnNameBeans.elementAt(i)).getAttribute("LABEL");
				String nomeColonna2 = null;
				if (((SourceBean) columnNameBeans.elementAt(i)).getAttribute("LABEL2") != null) {
					nomeColonna2 = (String) ((SourceBean) columnNameBeans.elementAt(i)).getAttribute("LABEL2");
				}
				if (sceltaNomeColonna != null && sceltaNomeColonna.equalsIgnoreCase("1")) {
					nomeColonna = nomeColonna2;
				}

				SourceBean columnNameBean = (SourceBean) columnNameBeans.elementAt(i);
				columnNames.addElement(nomeColonna);
			} // for (int i = 0; i < columnNameBeans.size(); i++)
	...
	public void delete(SourceBean request) {
		/*
		 * NON USATO - rimosso da GG il 28/9/2004. in caso di ripristino, usare il QueryExecutor e simili per avere
		 * anche il LOG!
		 * 
		 * DataConnection dc = null; DataConnectionManager dcm=null; SQLCommand cmdDelete = null; DataResult dr = null;
		 * String className = this.getClass().getName(); String pool;
		 * 
		 * try{
		 * 
...
} // public class AppuntamentiModule extends AbstractListModule
