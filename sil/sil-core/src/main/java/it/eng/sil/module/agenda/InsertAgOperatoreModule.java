...
public class InsertAgOperatoreModule extends AbstractModule {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(InsertAgOperatoreModule.class.getName());

	public InsertAgOperatoreModule() {
	}
	...
	public void service(SourceBean request, SourceBean response) {
		...
		try {
			...
			if (nro != null && nro.equals("0")) {
				// Savino 19/09/05: aggiunta gestione aggiornamento operatore
				// mantengo la vecchia superclasse .... andra' aggiornata con l'
				// AbstractSimpleModule
				if (request.containsAttribute("PRGSPI_DA_AGGIORNARE")) {
					...
					if (!esito.booleanValue()) {
						_logger.debug(className + "::udpate: nessuna riga aggiornata");

						// engErrorHandler.addError(new
						// EMFUserError(EMFErrorSeverity.WARNING, 10006));
						engErrorHandler.addError(
								new EMFUserError(EMFErrorSeverity.ERROR, MessageCodes.General.OPERATION_FAIL));
						return;
					}
				} else {
				...
				// Savino 19/09/05: vengono inseriti due errori nella risposta.
				// Tolgo quello generico
				// engErrorHandler.addError(new
				// EMFUserError(EMFErrorSeverity.ERROR,
				// MessageCodes.General.OPERATION_FAIL));
