...
public class InsertRichSosp extends AbstractSimpleModule {
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		try {
			...
			BigDecimal PRGRICHSOSPENSIONE = doNextVal(request, response);

			if (PRGRICHSOSPENSIONE == null) {
				throw new Exception("Impossibile leggere S_CM_RICH_SOSPENSIONE.NEXTVAL");
			}

			request.delAttribute("PRGRICHSOSPENSIONE");
			request.delAttribute("strChiaveTabella");
			request.setAttribute("PRGRICHSOSPENSIONE", PRGRICHSOSPENSIONE.toString());
			request.setAttribute("strChiaveTabella", PRGRICHSOSPENSIONE.toString());

			this.setSectionQueryInsert("QUERY_INSERT_RICH_SOSP");
			ret = doInsert(request, response);

			if (!ret) {
				throw new Exception("impossibile inserire in CM_RICH_SOSPENSIONE in transazione");
			} else {
			...
			response.delAttribute("PRGRICHSOSPENSIONE");
			response.setAttribute("PRGRICHSOSPENSIONE", PRGRICHSOSPENSIONE);
