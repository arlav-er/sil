...
public class ApiListaCancellati {
	...
	public ApiListaCancellati(SourceBean conf) {
		this.config = conf;
	}
	...
	public void start() throws Exception {
		try {
			...
			eng.setReportTitle("Lista Cancellati");
	...
	private void testata(Area area) throws Exception {
		...
		p = text.addParagraph();
		p.setHeight(0);
		tp = p.addTextPart("Lista Cancellati");
		s.setStyle(tp);
		tp.setFontSize(10);
		tp.setFontName("Times New Roman");
		tp.setBold(true);
	...
	private void stampaFiltriRicerca(Area ph) throws Exception {
		...
		if (!getDataComitatoDa().equals("")) {
			textFinale = textFinale + " Data comitato da " + getDataComitatoDa() + ";";
		}

		if (!getDataComitatoA().equals("")) {
			textFinale = textFinale + " Data comitato a " + getDataComitatoA() + ";";
		}
	...
	private void stampaIntestazione(Area ph, Vector rows) throws Exception {
		...
		t = addText(section, "Telefono\ndomicilio", s, 1300);
		t.setHeight(600);

		t = addText(section, "Data\ncessazione", s, 1480);
		t.setHeight(600);

		t = addText(section, "Azienda", s, 1660);
		t.setHeight(600);
		...
		t = addText(section, "Tipo\nlista", s, 2380);
		t.setHeight(600);

		t = addText(section, "Motivo\ndecadenza", s, 2500);
		t.setHeight(600);
	...
	private void azione(Area details, SourceBean infoAzione) throws Exception {
		...
		String frazione = StringUtils.getAttributeStrNotNull(infoAzione, "frazione");
		String cap = StringUtils.getAttributeStrNotNull(infoAzione, "strcapdom");
		String indirizzoCompleto = indirizzo;
		if (!cap.equals("")) {
			indirizzoCompleto = indirizzoCompleto + " " + cap;
		}
		if (!frazione.equals("")) {
			indirizzoCompleto = indirizzoCompleto + " " + frazione;
		}
		String telefono = StringUtils.getAttributeStrNotNull(infoAzione, "strteldom");
		String dataFineRapporto = StringUtils.getAttributeStrNotNull(infoAzione, "dataCess");
		...
		String motivoDecad = StringUtils.getAttributeStrNotNull(infoAzione, "decadenza");
		...
		t = addText(section, comune, 8, 750, 250);
		t.setCanGrow(true);

		t = addText(section, indirizzoCompleto, 8, 1000, 300);
		t.setCanGrow(true);

		t = addText(section, telefono, 8, 1300, 180);
		t.setCanGrow(true);

		t = addText(section, dataFineRapporto, 8, 1480, 180);
		t.setCanGrow(true);

		t = addText(section, azienda, 8, 1660, 340);
		t.setCanGrow(true);

		t = addText(section, datInizio, 8, 2000, 180);
		t.setCanGrow(true);

		t = addText(section, datFine, 8, 2180, 180);
		t.setCanGrow(true);

		t = addText(section, tipoMob, 8, 2360, 140);
		t.setCanGrow(true);

		t = addText(section, motivoDecad, 8, 2500, 200);
		t.setCanGrow(true);
	}
	...
	public void setDataComitatoDa(String val) {
		this.dataComitatoDa = val;
	}

	public String getDataComitatoDa() {
		return this.dataComitatoDa;
	}

	public void setDataComitatoA(String val) {
		this.dataComitatoA = val;
	}

	public String getDataComitatoA() {
		return this.dataComitatoA;
	}
