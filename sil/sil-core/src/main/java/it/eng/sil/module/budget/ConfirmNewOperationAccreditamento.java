 ...
 *
 */
public class ConfirmNewOperationAccreditamento extends AbstractSimpleModule {
	...
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger
			.getLogger(ConfirmNewOperationAccreditamento.class.getName());

	public void service(SourceBean serviceRequest, SourceBean serviceResponse) throws Exception {

		// Modificato per errore su delete
		_logger.debug("Sono dentro la classe ConfirmNewOperationAccreditamento ");
		...
		try {
			...
			String prgAzioneMisuraSel = (String) serviceReq.getAttribute("PrgAzioniMisuraSel");
			String dataDalSel = (String) serviceReq.getAttribute("dataDalSel");
			String dataAlSel = (String) serviceReq.getAttribute("dataAlSel");
			...
			String comune = (String) serviceReq.getAttribute("codComuneSel");
			String noteStampa = (String) serviceReq.getAttribute("noteStampaSel");

			String statement = SQLStatements.getStatement("STORED_OPERAZIONE_INSERT_ACCREDITAMENTO");
			...
			parameters.add(conn.createDataField("PAR_PRGAZIONI", Types.BIGINT, new BigInteger(prgAzioneMisuraSel)));
			command.setAsInputParameters(paramIndex++);

			parameters.add(
					conn.createDataField("PAR_DATINIZIOVAL", Types.DATE, DateUtils.convertStringToDate((dataDalSel))));
			command.setAsInputParameters(paramIndex++);

			parameters.add(
					conn.createDataField("PAR_DATFINEVAL", Types.DATE, DateUtils.convertStringToDate((dataAlSel))));
			command.setAsInputParameters(paramIndex++);
			...
			if (!codiceRit.equals("0")) {
				...
				case -1: // errore generico sql
					...
					msg = "Inserisci nuovo Accreditamento: errore generico";
					break;
				case 1: // errore prospetto duplicato
					msgCode = MessageCodes.General.OPERATION_FAIL;
					msg = "Inserisci nuovo Accreditamento: budget duplicato";
					break;
				case 10:
					msgCode = MessageCodes.General.OPERATION_FAIL;
					msg = errCode;
				...
				case 91:
					msgCode = MessageCodes.General.OPERATION_FAIL;
					msg = "Impossibile inserire valore duplicato";
					break;
				...
				default:
					msgCode = MessageCodes.General.OPERATION_FAIL;
					msg = "Inserisci nuovo Accreditamento: errore di ritorno non ammesso.";
				}
			...
			requestContainer.setAttribute("COMUNE", comune);
			requestContainer.setAttribute("noteStampa", noteStampa);
			// serviceResponse.setAttribute("tipoOperazioneSel",)
			requestContainer.setAttribute("cfSel", codiceFiscaleSel);
			requestContainer.setAttribute("codSedeSel", codSedeSel);
