...
import it.eng.sil.coop.webservices.mystage.patto.custom.PattiLavoratore.ProfilingPatto;
import it.eng.sil.module.patto.PartecipanteGGUtils;
import it.eng.sil.util.amministrazione.impatti.PattoBean;

public class GetPattoLavoratore implements GetPattoLavoratoreInterface {

	static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(GetPattoLavoratore.class.getName());
	private static ObjectFactory factory = new ObjectFactory();
	private static final String SERVIZIO = "GetPattoLavoratore";

	private static final String ESITO_OK = "00";
	private static final String DESC_OK = "OK";

	private static String QUERY_STRING_AZIONI_ASSOCIATE = "SELECT pc.prgpercorso, pc.prgcolloquio, pc.prgazioni, az.strdescrizione AS descAzione, de_azione_ragg.strdescrizione AS descObiettivoAzione, "
			+ " az.codazionesifer, az.codsottoazione, az.prgazioneragg, "
			+ " az.flgformazione, pc.flggruppo, pc.numpartecipanti, to_char(co.datcolloquio, 'dd/mm/yyyy') AS datcolloquio, to_char(pc.dateffettiva, 'dd/mm/yyyy') AS dateffettiva, "
			...
			+ " FROM or_percorso_concordato pc JOIN or_colloquio co ON (pc.prgcolloquio = co.prgcolloquio) JOIN de_azione az ON (az.prgazioni = pc.prgazioni) JOIN de_azione_ragg ON (az.prgazioneragg = de_azione_ragg.prgazioniragg) "
			+ " JOIN am_lav_patto_scelta ON (to_number(am_lav_patto_scelta.strchiavetabella) = pc.prgpercorso) "
			+ " WHERE trunc(SYSDATE) <= trunc(az.datfineval) and co.cdnlavoratore = ? and am_lav_patto_scelta.prgpattolavoratore = ? "
			+ " and pc.codesito not in ('NA', 'RIF') " + " order by prgazioni";
	...
	public String getDati(String userName, String password, String codiceFiscale, String dataInizioTirocinio) {
		...
		try {
			...
			qExec.setStatement(SQLStatements.getStatement("GET_Dati_Patto_WS_MyStage"));
			qExec.setType(QueryExecutorObject.SELECT);
			...
			if ((pattoBeanRows == null) || (pattoBeanRows.getAttributeAsVector("ROW").size() == 0)) {
				codiceStr = "05";
				descrizioneStr = "Nessun patto protocollato";
				throw new MyStageException("", codiceStr, descrizioneStr);
			}
	...
	private static ErroreMyStage getPatti(QueryExecutorObject qExec, DataConnection dc, PattiLavoratore patti,
		...
		if (pattiBeanRows != null) {
			...
			codtipopatto = (String) pattiBeanRow.getAttribute("CODTIPOPATTO");
			codCodificaPatto = (String) pattiBeanRow.getAttribute("CODCODIFICAPATTO");
			datScadenza = (String) pattiBeanRow.getAttribute("DATSCADCONFERMA");
			if (datScadenza != null) {
			...
			if ((codtipopatto != null && codtipopatto.equalsIgnoreCase(PattoBean.DB_MISURE_GARANZIA_GIOVANI))
					|| (isPattoGG)) {
				/* recupero della data adesione GG relativa al patto solo se MGG */
				try {
					...
					if (adesioneAttivaBeanRows != null
							&& !adesioneAttivaBeanRows.getAttributeAsVector("ROW").isEmpty()) {
							...
							if (erroreNoAdesioneA02) {
								...
								logger.error(
										"Impossibile recuperare i dati relativi all'adesione GG, esistono diverse azioni A02 con la stessa data adesione massima: "
												+ cdnLavoratore);
								return new ErroreMyStage(MessageCodes.YG.ERR_WS_POLITICA_ATTIVA_MULTIPLA_MYSTAGE,
										params);
							}
						}
						...
						if (adesioneAttivaA2RRows != null
								&& !adesioneAttivaA2RRows.getAttributeAsVector("ROW").isEmpty()) {
			...
			profilingPatto.setPattoProgressivo(prgPattoLavoratore.toBigInteger());
			...
			profilingPatto.setTipoMisuraPatto(codtipopatto);
			profilingPatto.setTipoCodificaPatto(codCodificaPatto);
			...
			profilingPatto.setDataScadenzaPatto(datScadenzaGregorian);
			...
			try {
				...
				qExec.setInputParameters(param);
				qExec.setType(QueryExecutorObject.SELECT);
				qExec.setStatement(QUERY_STRING_AZIONI_ASSOCIATE);
	...
	private static ErroreMyStage getPoliticheAttive(QueryExecutorObject qExec, DataConnection dc,
		...
		if (politicheAttiveBeanRows != null) {
			...
			String descObiettivoAzione = null;
			...
			for (int i = 0; i < politicheAttiveBeanVector.size(); i++) {
				...
				descObiettivoAzione = (String) politicheAttiveBeanRow.getAttribute("DESCOBIETTIVOAZIONE");
				...
				if (prgAzione != null) {
					politicaAttiva.setPrgAzione(prgAzione.toBigInteger());
				}
				...
				if (descObiettivoAzione != null) {
					politicaAttiva.setObiettivo(descObiettivoAzione);
				}
				if (descAzione != null) {
					politicaAttiva.setAzione(descAzione);
				}
	...
	private boolean isAutenticazioneValida(String userName, String password, TransactionQueryExecutor tex)
		...
		params[0] = SERVIZIO;
		try {
			SourceBean logon = (SourceBean) tex.executeQuery("WS_LOGON_BLEN", params, "SELECT");
