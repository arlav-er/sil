...
public class InsertRichGrad extends AbstractSimpleModule {
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		try {
			transExec = new TransactionQueryExecutor(Values.DB_SIL_DATI, this);
			this.enableTransactions(transExec);

			transExec.initTransaction();

			BigDecimal PRGRICHGRADUALITA = doNextVal(request, response);

			if (PRGRICHGRADUALITA == null) {
				throw new Exception("Impossibile leggere S_CM_RICH_GRAD.NEXTVAL");
			}

			request.delAttribute("PRGRICHGRADUALITA");
			request.delAttribute("strChiaveTabella");
			request.setAttribute("PRGRICHGRADUALITA", PRGRICHGRADUALITA.toString());
			request.setAttribute("strChiaveTabella", PRGRICHGRADUALITA.toString());

			this.setSectionQueryInsert("QUERY_INSERT_RICH_GRAD");
			ret = doInsert(request, response);

			if (!ret) {
				throw new Exception("impossibile inserire in CM_RICH_PRGRICHGRADUALITA in transazione");
			} else {
			...
			response.delAttribute("PRGRICHGRADUALITA");
			response.setAttribute("PRGRICHGRADUALITA", PRGRICHGRADUALITA);
