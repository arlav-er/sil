...
public class MInserisciOrarioRichiesta extends AbstractSimpleModule {
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		Action insertOrariAction = new InsertElementAction(this);

		Collection elencoOrari = AbstractSimpleModule.getArgumentValues(request, "CODORARIO");

		CollectionExecutor orariExecutor = new CollectionExecutor(elencoOrari, insertOrariAction, "CODORARIO");

		Boolean updateFlg = new Boolean(false);
		String flgInvioCL = StringUtils.getAttributeStrNotNull(request, "FLGINVIOCL");
		numeroElementi = elencoOrari.size();
		...
		try {
			...
			orariExecutor.execute(request, response);
