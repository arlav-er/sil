...
public class BatchAvvASelezioni implements BatchRunnable, IBatchMDBConsumer {
	private BatchObject batchObject;
	private String[] parametri;
	private LogBatch logBatch;

	public BatchAvvASelezioni(BatchObject batchObject) throws Exception {
		this.batchObject = batchObject;
		String dir = ConfigSingleton.getLogBatchPath();
		String nomeFile = File.separator + "BatchAvvASelezioni";
		String data = "";
	...
	public void start() {
		try {
			...
			if (parametri[0] != null) {
				msgParametri = "Numero Max Mesi : " + parametri[0].toString();
				logBatch.writeLog(msgParametri);
			}
			...
			String p_numMaxMesi = parametri[0];
			...
			String ret = executeStored(p_numMaxMesi, p_ut);
	...
	String executeStored(String p_numMaxMesi, String p_ut) throws Exception {
		...
		try {
			...
			command = (StoredProcedureCommand) conn
					.createStoredProcedureCommand("{ call PG_INCROCIO.ASBatch_AvvASelezioneDaMov(?, ?, ?) }");
			...
			parameters.add(conn.createDataField("p_numMaxMesi", Types.INTEGER, p_numMaxMesi));
	...
	public boolean setParametri() {
		...
		parametri[0] = args[0]; // Numero Max Mesi
