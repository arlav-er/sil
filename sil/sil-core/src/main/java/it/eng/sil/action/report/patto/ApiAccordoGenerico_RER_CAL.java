 ...
 */
public class ApiAccordoGenerico_RER_CAL {
	Style campo;
	Style titoloSezione;
	Style etichetta;
	Style etichetta2;
	...
	private static final String CAL = "18";
	...
	public ApiAccordoGenerico_RER_CAL() {
	...
	public ApiAccordoGenerico_RER_CAL(String[] a) throws Exception {
		start();
	}
	...
	public void start() throws Exception {
		try {
			...
			// addBR(details, 40);
			enteAccreditatoNuovoAccordo(details, soggettiAccreditati);
			...
			for (int i = 0; i < azioni.size(); i++) {
				...
				if (!prestazione.equalsIgnoreCase(prestazione_before)) {
					...
					if (this.regione.equals(Properties.RER)) {
						t = ReportUtils.addText(section, "Prestazione: ", etichetta, LEFT_TITOLO_PUNTO_T_INTERMEDIO);
					} else {
						t = ReportUtils.addText(section, "Obiettivo: ", etichetta, LEFT_TITOLO_PUNTO_T_INTERMEDIO);
					}
				...
				if (!poc) {
					...
					section = details.addSection();
					if (this.regione.equals(Properties.RER)) {
						t = ReportUtils.addText(section, "Attività: ", etichetta, LEFT_TITOLO_PUNTO_T);
					} else {
						t = ReportUtils.addText(section, "Azione: ", etichetta, LEFT_TITOLO_PUNTO_T);
					}
	...
	private void enteAccreditatoNuovoAccordo(Area details, Vector soggetti) throws Exception {
	...
	private void paginaInformazioni(Area details, String cpi, String dataStipula, String dataScadenza,
		...
		if (!this.regione.equals(CAL) && dataScadenza != null) {
			tp = p.addTextPart("Il presente Accordo è valido sino alla data: ");
			tp.setFontSize(11);
			tp.setFontName("Times New Roman");
