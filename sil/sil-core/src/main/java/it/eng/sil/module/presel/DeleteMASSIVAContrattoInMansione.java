...
public class DeleteMASSIVAContrattoInMansione extends AbstractSimpleModule {
	public void service(SourceBean request, SourceBean response) {
		ReportOperationResult reportOperation = new ReportOperationResult(this, response);
		int idSuccess = this.disableMessageIdSuccess();
		int idFail = this.disableMessageIdFail();

		String prgStr = (String) request.getAttribute("PRGDISCONTRATTOCANCMASSIVA");
		String prg = null;
		int virgola;
		boolean ret = true;
	...
	private void setKeyinRequest(Object prg, SourceBean request) throws Exception {
		if (request.getAttribute("PRGDISCONTRATTO") != null) {
			request.delAttribute("PRGDISCONTRATTO");
		}
		request.setAttribute("PRGDISCONTRATTO", prg);
	}
