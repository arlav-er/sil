...
public class InsertProvv extends AbstractSimpleModule {
	private final String className = StringUtils.getClassName(this);
	private final String gestisciComputo = "Aggiornamento del Provvedimento effettuato con successo";
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		try {
			...
			if (request.containsAttribute("inserisciNew")) {
				...
				this.setSectionQueryInsert("QUERY_INSERT_PROVEDDIMENTO");
				success = this.doInsert(request, response);
				if (success) {
					gestisciDocumento(request);
				} else
					throw new Exception();
				if (codStatoAtto.equals("PR") && "1".equals(s_conf_provvedimento) && success == true) {
	...
	private boolean gestisciComputo(SourceBean request, ReportOperationResult reportOperation) throws Exception {
		...
		if (("-1").equals(codiceRit)) {
			...
			reportOperation.reportWarning(MessageCodes.CollocamentoMirato.ERRORE_AGG_L68_PROSPETTO_INESISTENTE,
					className, "gestione aggiornamento prospetto da computo");
			return true;
		} else if (("-5").equals(codiceRit)) {
			reportOperation.reportWarning(MessageCodes.CollocamentoMirato.WARNING_L68_CONV_LAV_PROSPETTO_AGGIORNATO,
					className, "gestione aggiornamento prospetto da computo", paramErrore);
			return true;
		} else {
