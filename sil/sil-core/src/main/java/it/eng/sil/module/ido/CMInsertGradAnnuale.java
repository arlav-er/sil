...
public class CMInsertGradAnnuale extends AbstractSimpleModule {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(CMInsertGradAnnuale.class.getName());
	...
	public void service(SourceBean request, SourceBean response) throws EMFInternalError {
		...
		try {
			...
			// creaazione della graduatoria e dei nominativi in graduatoria
			String prgGraduatoria = insertGradAnnuale(request, response, transExec);
			 ...
			 * (String)lavoratoriSB.getAttribute("DATRIFERIMENTO"); String datRifIscrizione = "30/04/1988";
			 * 
			 ...
			 * 
			 * if (numMesiAnzianita >= 0) { // calcolo i mesi di anzianità int numMesiAnzianitaOld =
			 * DBLoad.calcolaAnzianitaCM(cdnLavoratore, dataAnzianitaCM, "", 0, datRifIscrizione);
			 * 
			 * numMesiAnzianita = DBLoad.calcolaAnzianitaCM(cdnLavoratore, dataAnzianitaCM, "", 0, datRiferimento);
			 * 
			...
			it.eng.sil.util.TraceWrapper.debug(_logger,
					"CMInsertGradAnnuale::service(): Impossibile calcolare la graduatoria!", ex);
	...
	private String insertGradAnnuale(SourceBean request, SourceBean response, TransactionQueryExecutor txExec)
			throws Exception {
		...
		String errCode = "";
		// parametri
		String p_annoGrad = StringUtils.getAttributeStrNotNull(request, "ANNOGRAD");
		String p_dataRifGrad = StringUtils.getAttributeStrNotNull(request, "DATARIFGRAD");
		String p_annoRifReddito = StringUtils.getAttributeStrNotNull(request, "ANNORIFREDDITO");
		String p_codMonoTipoGrad = StringUtils.getAttributeStrNotNull(request, "CODMONOTIPOGRAD");
		String p_strnote = StringUtils.getAttributeStrNotNull(request, "STRNOTE");
		String p_cdnUtente = String.valueOf(user.getCodut());
		String p_ambTerr = StringUtils.getAttributeStrNotNull(request, "PROVINCIA_ISCR");

		int paramIndex = 0;
		ArrayList parameters = null;
		conn = txExec.getDataConnection();

		statementSB = (SourceBean) getConfig().getAttribute("CM_INSERT_GRAD_ANNUALE");
		...
		// 2 anno graduatoria
		parameters.add(conn.createDataField("annoGrad", java.sql.Types.VARCHAR, p_annoGrad));
		command.setAsInputParameters(paramIndex++);
		// 3 data riferimento
		parameters.add(conn.createDataField("dataRifGrad", java.sql.Types.VARCHAR, p_dataRifGrad));
		command.setAsInputParameters(paramIndex++);
		// 4 anno reddito
		parameters.add(conn.createDataField("annoRifReddito", java.sql.Types.VARCHAR, p_annoRifReddito));
		command.setAsInputParameters(paramIndex++);
		// 5 tipo iscr cm
		parameters.add(conn.createDataField("codMonoTipoGrad", java.sql.Types.VARCHAR, p_codMonoTipoGrad));
		command.setAsInputParameters(paramIndex++);
		// 6 note
		parameters.add(conn.createDataField("strnote", java.sql.Types.VARCHAR, p_strnote));
		command.setAsInputParameters(paramIndex++);
		// 7 chiave crypt
		...
		command.setAsInputParameters(paramIndex++);
		// 9 provincia iscrizione (ambito territoriale)
		parameters.add(conn.createDataField("p_ambitoTerr", java.sql.Types.VARCHAR, p_ambTerr));
		command.setAsInputParameters(paramIndex++);
		// 10 prg nuova graduatoria
		...
		if (!codiceRit.equals("0")) {
			...
			case -1: // errore generico sql
				...
				break;
			case 1: // errore graduatoria già presente
				msgCode = MessageCodes.CollocamentoMirato.ERROR_GRAD_ANNUALE_PRESENTE;
				msg = "Graduatoria annuale già presente";
	 ...
	 * getRequestContainer().getSessionContainer().getAttribute("_ENCRYPTER_KEY_"); String p_flgRigenera = "N";
	 * 
	 * parameters = new ArrayList(5); // 1.Parametro di Ritorno parameters.add(conn.createDataField("codiceRit",
