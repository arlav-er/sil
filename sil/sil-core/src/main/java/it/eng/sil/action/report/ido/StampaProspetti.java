...
public class StampaProspetti extends AbstractSimpleReport {
	...
	Engine executeQueryAdesioni(SourceBean request, SourceBean response) throws Exception {
		...
		String numScoperturaMasso_Fisioterapista = "N";

		String numScoperturaCentra;
		String numScoperturaFisioterapista;
		...
		String tolleranza = StringUtils.getAttributeStrNotNull(request, "TOLLERANZA");
		...
		for (int i = 0; i < vecAziProspetti.size(); i++) {
			...
			// numScoperturaMasso = vecAziProspetti.
			// numScoperturaFisioterapista =

			numScoperturaFisioterapista = (String) rowProspetti.getAttribute("NUMSCOPERTURAMASSOFISIO");
			numScoperturaCentra = (String) rowProspetti.getAttribute("NUMSCOPERTURANVED");

			if (numScoperturaFisioterapista == null) {
				numScoperturaFisioterapista = "0";
			}

			if (numScoperturaCentra == null) {
				numScoperturaCentra = "0";
			}

			if (Float.valueOf(numScoperturaFisioterapista.trim()).floatValue() > 0
					|| Float.valueOf(numScoperturaCentra.trim()).floatValue() > 0) {
				numScoperturaMasso_Fisioterapista = "S";
			}
			...
			if (scopertura != null && !scopertura.equals("")) {
				...
				if (st.hasMoreElements()) {
					...
					if ((tolleranza.equals("1") && (risDis > 0.50 || risArt18 > 0.50)) || tolleranza.equals("0")
							|| numScoperturaMasso_Fisioterapista.equals("S")) {
		...
		String numNved_Masso = (String) request.getAttribute("numNved_Masso");
		// Proviamo se c'è la conf per Centralisnista e Masso
		if ((numNved_Masso != null) && (!numNved_Masso.equals("")) && (numNved_Masso.equals("1"))) {
			ApiStampaProspetti_Central_Masso report = new ApiStampaProspetti_Central_Masso((SourceBean) getConfig());
			...
			report.setTolleranza(tolleranza);
			...
			ApiStampaProspetti report = new ApiStampaProspetti((SourceBean) getConfig());
