...
public class DelRipetizione extends AbstractModule {
	private String className = this.getClass().getName();

	private String dataFineDef = "01/01/2100";

	public DelRipetizione() {
	}
		...
		try {
			...
			if (tipoUpdf.equals("2")) {
				...
				esito = (Boolean) QueryExecutor.executeQuery(getRequestContainer(), getResponseContainer(), pool,
						statement, "UPDATE");
				LogUtils.logDebug("MDelRipetizione", "Caso2: UPD -Modifica Regola Festivo", statement);
				// b. Insert
				gc.set(annoDB, meseDB - 1, giornoDB);
				gc.add(Calendar.DATE, 1);
				g = gc.get(Calendar.DATE);
				...
				esitoIns = (Boolean) QueryExecutor.executeQuery(getRequestContainer(), getResponseContainer(), pool,
						statement, "INSERT");
				LogUtils.logDebug("MDelRipetizione", "Caso 2: INS - Inserimento Regola Festivo", statement);
			...
			result.reportFailure(e, className, "Errore durante la cancellazione della regola per il giorno festivo");
		}
