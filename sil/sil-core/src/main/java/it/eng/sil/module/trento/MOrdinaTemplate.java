...
public class MOrdinaTemplate extends AbstractSimpleModule {
	
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(MOrdinaTemplate.class.getName());
	private final String className = StringUtils.getClassName(this);
	
	
	public MOrdinaTemplate() {
	}
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		try {
			...
			String ordinamento = (String) request.getAttribute("ord_templ");
			...
			for (int j = 0; j < tokenCount; j++) {
				...
				Boolean res = (Boolean)trans.executeQuery("QUERY_UPDATE_ORDINAMENTO_TEMPLATE", params, "UPDATE");
				_logger.debug(className + "res: " + res);
		        if (!res.booleanValue()) throw new Exception ("Impossibile inserire ordinamento");
			...
			it.eng.sil.util.TraceWrapper.debug(_logger, "MOrdinaTemplate.service()", ex);
			response.setAttribute("aggiornato", "false");
			reportOperation.reportFailure(errorCode);
		}
	}
