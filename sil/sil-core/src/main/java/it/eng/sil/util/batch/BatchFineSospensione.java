...
public class BatchFineSospensione implements BatchRunnable, IBatchMDBConsumer {
	private BatchObject batchObject;
	private String[] parametri;
	private LogBatch logBatch;
	private String dataBatch = "";

	public BatchFineSospensione(BatchObject batchObject) throws Exception {
		this.batchObject = batchObject;
		String dir = ConfigSingleton.getLogBatchPath();
		String nomeFile = File.separator + "BatchFineSospensione";
		String data = "";
	...
	public void start() {
		try {
			logBatch.writeLog(
					"=========== Avvio batch per ricalcolare lo stato occupazionale a seguito della fine sospensione ===========");
			...
			Vector rows = DBLoad.getMovimentiFineSospensione(parametri[0]);

			msg = "Sono stati letti i movimenti arrivati a fine sospensione.";
			...
			for (int i = 0; i < rows.size(); i++) {
				try {
					...
					dataCalcolo = row.getAttribute("DATINIZIOMOV").toString();
					msg = "Lavoratore con identificativo : " + cdnlavoratoreCurr.toString() + " Codice fiscale : "
							+ row.getAttribute("STRCODICEFISCALE").toString() + " Movimento con data inizio : "
							+ dataCalcolo;
					logBatch.writeLog(msg);
					msg = "";
					...
					if (!canEditLav) {
						...
						msg = "";
						msg = "Il nuovo stato occupazionale è:" + statoOccFinale.getDescrizioneCompleta();
						logBatch.writeLog(msg);
			...
			msg = "";
			logBatch.writeLog(msg);
			logBatch.writeLog("Il batch fine sospensione è terminato.");
		} catch (Exception e) {
	...
	public void setParametri() {
		// datalancio nonTrattati true user profilo gruppo quando viene chiamato dalla pagina
		parametri = new String[6];

		String[] args = this.batchObject.getParams();
		if (args.length == 6) { // viene chiamato dalla pagina
			parametri[0] = args[0]; // data fine range (usata)
			parametri[1] = args[1]; // non trattati - anche trattati
			parametri[2] = args[2]; // forzaReinserimento
			parametri[3] = args[3]; // user
			parametri[4] = args[4]; // profilo user
			parametri[5] = args[5]; // gruppo user
		} else { // viene schedulato
			parametri[0] = dataBatch;
			parametri[1] = args[0]; // non trattati - anche trattati
			parametri[2] = args[1];// forzaReinserimento
			parametri[3] = args[2];// user
			parametri[4] = args[3]; // profilo user
			parametri[5] = args[4]; // gruppo user
		}
	}
}