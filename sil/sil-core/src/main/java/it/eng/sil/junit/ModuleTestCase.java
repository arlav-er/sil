...
import com.engiweb.framework.dispatching.module.ModuleException;
...
public class ModuleTestCase {
	...
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(ModuleTestCase.class.getName());
	...
	private ModuleTestCase() {
	}

	public ModuleTestCase(String _moduleName) {
		...
		_logger.debug("*** Test del modulo " + moduleName);
		...
		try {
			...
			it.eng.sil.util.TraceWrapper.debug(_logger, "Errore in ModuleTestCase", e3);
		...
		try {
			serviceRequest.setAttribute("AF_MODULE_NAME", _moduleName);
	...
	public void execService() throws Exception {
		...
		try {
			...
			it.eng.sil.util.TraceWrapper.fatal(_logger,
					"Errore in ModuleTestCase. Errore nel caricamento dell'utente in sessione", e3);
		...
		try {
			...
			it.eng.sil.util.TraceWrapper.debug(_logger, "Errore in ModuleTestCase.", e);
		...
		try {
			...
			((RequestContextIFace) module).setRequestContext(requestContext);

			module.service(serviceRequest, serviceResponse);
		} catch (ModuleException e1) {

			it.eng.sil.util.TraceWrapper.fatal(_logger,
					"Errore in ModuleTestCase. Errore nell'esecuzione del modulo" + moduleName, e1);

			throw e1;
		} catch (Exception e2) {
			it.eng.sil.util.TraceWrapper.fatal(_logger,
					"Errore in ModuleTestCase. Errore generico nell'esecuzione del modulo" + moduleName, e2);
