...
public class StampaLavObbligoIstruzione extends AbstractSimpleReport {
	...
	public void service(SourceBean request, SourceBean response) {
		...
		if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
			...
			if (tipoFile != null)
				setStrNomeDoc("LavObbligoIstruzione." + tipoFile);
			else
				setStrNomeDoc("LavObbligoIstruzione.pdf");
			setStrDescrizione("Lavoratori con obbligo istruzione");
			...
			try {
				...
				if ((salva != null) && salva.equalsIgnoreCase("true")) {
					eng = executeQueriesLavOI(request, response);
	...
	com.inet.report.Engine executeQueriesLavOI(SourceBean request, SourceBean response) throws Exception {
		String tipoFile = (String) request.getAttribute("tipoFile");
		SourceBean listaLavOI = doDynamicSelect(request, response);

		Vector vLavOI = listaLavOI.getAttributeAsVector("ROW");

		ApiLavObbligoIstruzione report = new ApiLavObbligoIstruzione((SourceBean) getConfig());
