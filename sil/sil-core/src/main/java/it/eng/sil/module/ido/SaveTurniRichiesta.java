...
public class SaveTurniRichiesta extends AbstractSimpleModule {
	public void service(SourceBean request, SourceBean response) {
		...
		Action insertTurniAction = new InsertElementAction(this);

		Collection elencoTurni = AbstractSimpleModule.getArgumentValues(request, "CODTURNO");

		CollectionExecutor turniExecutor = new CollectionExecutor(elencoTurni, insertTurniAction, "CODTURNO");

		numeroElementi = elencoTurni.size();
		if (numeroElementi > 1) {
		...
		try {
			...
			turniExecutor.execute(request, response);
