package it.eng.sil.action.report.scadenziario;
...
public class ApiLavRicontattare {
	...
	public ApiLavRicontattare(SourceBean conf) {
		this.config = conf;
	}
	...
	public void start() throws Exception {
		try {
			...
			eng.setReportTitle("Lavoratori da Ricontattare");
	...
	private void stampaIntestazione(Area ph, Vector rows, boolean flgMatchingPesato) throws Exception {
		...
		t = addText(section, "Codice Fiscale", s, LEFT + 610);
		t.setHeight(600);

		t = addText(section, "Data di \nNascita", s, LEFT + 1020);
		t.setHeight(600);

		t = addText(section, "Indirizzo Domicilio ", s, LEFT + 1240);
		t.setHeight(600);

		t = addText(section, "Comune Domicilio ", s, LEFT + 1760);
		t.setHeight(600);

		t = addText(section, "Data \nContatto", s, LEFT + 2120);
		t.setHeight(600);
		t = addText(section, "Data \nScadenza", s, LEFT + 2330);
		t.setHeight(600);
	...
	private void azione(Area details, SourceBean infoAzione) throws Exception {
		...
		String strCf = it.eng.afExt.utils.StringUtils.getAttributeStrNotNull(infoAzione, "STRCODICEFISCALE");

		String datNasc = it.eng.afExt.utils.StringUtils.getAttributeStrNotNull(infoAzione, "DATNASC");

		String indirizzoDom = it.eng.afExt.utils.StringUtils.getAttributeStrNotNull(infoAzione, "strindirizzodom");

		if ("&nbsp;".equals(indirizzoDom)) {
			indirizzoDom = "";
		}

		String STRcittadomi = it.eng.afExt.utils.StringUtils.getAttributeStrNotNull(infoAzione, "STRcittadomi");

		String datContatto = it.eng.afExt.utils.StringUtils.getAttributeStrNotNull(infoAzione, "STRDATCONTATTO");
		String datScadenza = it.eng.afExt.utils.StringUtils.getAttributeStrNotNull(infoAzione, "STRDATENTROIL");
		...
		t = addText(section, strNome, 8, LEFT + 280, 300);
		t.setCanGrow(true);

		t = addText(section, strCf, 8, LEFT + 610, 350);
		t.setCanGrow(true);

		t = addText(section, datNasc, 8, LEFT + 1020, 200);
		t.setCanGrow(true);

		t = addText(section, indirizzoDom, 8, LEFT + 1240, 500);
		t.setCanGrow(true);

		t = addText(section, STRcittadomi, 8, LEFT + 1760, 350);
		t.setCanGrow(true);

		t = addText(section, datContatto, 8, LEFT + 2120, 200);
		t.setCanGrow(true);

		t = addText(section, datScadenza, 8, LEFT + 2330, 200);
		t.setCanGrow(true);
