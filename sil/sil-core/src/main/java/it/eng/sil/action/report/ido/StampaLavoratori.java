...
public class StampaLavoratori extends AbstractSimpleReport {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(StampaLavoratori.class.getName());
	...
	public void service(SourceBean request, SourceBean response) {
		...
		if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
			...
			if (tipoFile != null)
				setStrNomeDoc("StampaLavoratori." + tipoFile);
			else
				setStrNomeDoc("StampaLavoratori.pdf");
			setStrDescrizione("Stampa Lavoratori");
	...
	Engine executeQueryAdesioni(SourceBean request, SourceBean response) throws Exception {
		...
		String eta = Utils.notNull(request.getAttribute("eta"));

		String etaMaggiore = Utils.notNull(request.getAttribute("etaMaggiore"));
		String etaUguale = Utils.notNull(request.getAttribute("etaUguale"));
		int intEtaMaggiore = 0;
		int intEtaUguale = 0;
		if (!etaMaggiore.equals(""))
			intEtaMaggiore = Integer.parseInt(etaMaggiore);
		if (!etaUguale.equals(""))
			intEtaUguale = Integer.parseInt(etaUguale);

		Vector finale = new Vector();
		SourceBean lista = null;
		Vector vecAdesioniChiamata = listaAdesioniChiamata.getAttributeAsVector("ROW");

		int sizeVec = vecAdesioniChiamata.size();

		for (int i = 0; i < sizeVec; i++) {
			SourceBean richiesta = (SourceBean) vecAdesioniChiamata.get(i);

			String dataNascita = (String) richiesta.getAttribute("DATNASC");
			String dataOggi = (String) richiesta.getAttribute("DATOGGI");
			...
			if (!dataNascita.equals("")) {
				giornoNasc = Integer.parseInt(dataNascita.substring(0, 2));
				meseNasc = Integer.parseInt(dataNascita.substring(3, 5));
				annoNasc = Integer.parseInt(dataNascita.substring(6));
			}

			if (!dataOggi.equals("")) {
				giornoOggi = Integer.parseInt(dataOggi.substring(0, 2));
				meseOggi = Integer.parseInt(dataOggi.substring(3, 5));
				annoOggi = Integer.parseInt(dataOggi.substring(6));
			}

			if ((meseOggi >= meseNasc) && (giornoOggi >= giornoNasc)) {
				etaLavoratore = annoOggi - annoNasc;
			} else {
				etaLavoratore = annoOggi - annoNasc - 1;
			}

			if (eta.equals("maggiore")) {
				if (intEtaMaggiore < etaLavoratore) {
					finale.addElement(richiesta);
				}
			} else if (eta.equals("uguale")) {
				if (intEtaUguale == etaLavoratore) {
					finale.addElement(richiesta);
				}
			}
		}

		// int size = finale.size();

		ApiStampaLavoratori report = new ApiStampaLavoratori((SourceBean) getConfig());

		report.setLavoratori(finale);
		...
		report.setEta(eta);
		report.setEtaMaggiore(etaMaggiore);
		report.setEtaUguale(etaUguale);
