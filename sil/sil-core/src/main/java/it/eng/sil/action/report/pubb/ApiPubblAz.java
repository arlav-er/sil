 ...
 */
public class ApiPubblAz {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(ApiPubblAz.class.getName());
	...
	public ApiPubblAz(String[] a) throws Exception {
		start();
	}
	...
	private void stampaIntestazioneCpi(Area area, SourceBean row) throws Exception {
		...
		tp = par.addTextPart(strTitolo);
		tp.setFontName("Time New Roman");
		tp.setFontSize(14);
		tp.setBold(true);
		...
		sec = area.addSection();
		l = sec.addHorizontalLine(0, 0, sec.getWidth());
		try {
			l.setHeight(50);
		} catch (Exception e) {
		}
	...
	private void stampaPubblicazione(Area details, SourceBean row) throws Exception {
		...
		String strRichiesta = "Cod. " + numRichiesta + "/" + (BigDecimal) (row.getAttribute("NUMANNO")) + " del "
				+ (String) (row.getAttribute("DATRICHIESTA")) + " valida fino al "
				+ (String) (row.getAttribute("DATSCADENZA"));
		BigDecimal numProfiliRich = (BigDecimal) row.getAttribute("NUMPROFRICHIESTI");
		String strMansionePub = (String) row.getAttribute("STRMANSIONEPUBB");
		StringBuffer strMansioni = new StringBuffer();
		Vector vMansioniRic = row.getAttributeAsVector("MANSIONI.ROWS.ROW");
		for (int i = 0; i < vMansioniRic.size(); i++) {
			SourceBean sbMansione = (SourceBean) vMansioniRic.get(i);
			strMansioni.append((String) sbMansione.getAttribute("CODMANSIONE") + " ");
			strMansioni.append((String) sbMansione.getAttribute("STRDESCRIZIONE"));
			strMansioni.append("\n");
		}
		//
		...
		text = sec.addText(100, sec.getY(), sec.getWidth() - 100, 200);
		text.setCanGrow(true);
		par = text.addParagraph();
		...
		text = sec.addText(3600, sec.getY(), 400, 200);
		text.setCanGrow(true);
		par = text.addParagraph();
		tp = par.addTextPart(numProfiliRich.toString());
		tp.setFontName("Time New Roman");
		tp.setFontSize(10);
		tp.setBold(true);

		//
		text = sec.addText(4000, sec.getY(), sec.getWidth() - 4000, 200);
		text.setCanGrow(true);
		par = text.addParagraph();
		...
		//
		text = sec.addText(5300, sec.getY(), sec.getWidth() - 5300, 200);
		text.setCanGrow(true);
		par = text.addParagraph();
		tp = par.addTextPart(strMansioni.toString());
		tp.setFontName("Time New Roman");
		tp.setFontSize(10);
		tp.setBold(true);
		addBR(details, 20);
		...
		if (row.getAttribute("STRLUOGOLAVORO") != null) {
			sec = details.addSection();
			text = sec.addText(100, sec.getY(), 1900, 200);
			text.setCanGrow(true);
			par = text.addParagraph();
			...
			text = sec.addText(2200, sec.getY(), sec.getWidth() - 2200, 200);
			par = text.addParagraph();
		...
		if (row.getAttribute("STRRIFCANDIDATURAPUBB") != null) {
			//
			sec = details.addSection();
			text = sec.addText(100, 0, 1900, 200);
			text.setCanGrow(true);
			par = text.addParagraph();
			...
			text = sec.addText(2200, 0, sec.getWidth() - 2200, 200);
			par = text.addParagraph();
			text.setCanGrow(true);
			tp = par.addTextPart(
		...
		//
		if (strDiffusioni != null) {
			sec = details.addSection();
			text = sec.addText(100, 0, 1900, 200);
			text.setCanGrow(true);
			par = text.addParagraph();
			tp = par.addTextPart("Tipo di diffusione ");
			tp.setFontName("Time New Roman");
			tp.setFontSize(10);
			tp.setItalic(true);
			tp.setBold(false);
			...
			par = text.addParagraph();
			text.setCanGrow(true);
			tp = par.addTextPart(strDiffusioni.toString());
			tp.setFontName("Time New Roman");
			tp.setFontSize(10);
			tp.setBold(true);
