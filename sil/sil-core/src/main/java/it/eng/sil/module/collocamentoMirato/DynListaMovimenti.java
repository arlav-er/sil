...
public class DynListaMovimenti implements IDynamicStatementProvider {
	public String getStatement(RequestContainer requestContainer, SourceBean config) {
		...
		String query = "Select  Mov.Prgmovimento Prgmov, Mov.Prgmovimentoprec Prgmovimentoprec, "
				...
				+ "And Com.Codprovincia = Prov.Codprovincia " + " AND (Mov.Codtipomov = 'CES' "
				+ " OR (Mov.Codtipomov = 'AVV' AND Mov.CODTIPOCONTRATTO = 'Z.09.02' )) " + "And Mov.Prgazienda = "
				+ req.getAttribute("PRGAZIENDA").toString() + " And Mov.Prgunita = "
				+ req.getAttribute("PRGUNITA").toString() + " AND Mov.Datiniziomov >= to_DATE('"
				+ req.getAttribute("DATINIZIORAPP").toString() + "', 'DD/MM/YYYY') " + " And Mov.Cdnlavoratore = "
