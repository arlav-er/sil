...
class LogServiceClientSoapDB {

	// private static final String TABELLA_DEFAULT = "TS_WS_TRACCIAMENTO";
	/* fs20150226 - start */
	private String TABELLA_DEFAULT = "TS_WS_TRACCIAMENTO";
	private String SEQUENCE_DEFAULT = "TS_WS_TRACCIAMENTO";
	/* fs20150226 - end */
	...
	private static final String UPDATE_SQL_SERVICE_RESP_WITH_CF = "update %TABELLA% "
			+ " set codesito = ?, prgwstracciamentoresp = ?, dtmresp = sysdate , strcodicefiscalelav = ?"
			+ " where prgwslogservizi = ?";

	private static final Logger log = Logger.getLogger(LogServiceClientSoapDB.class.getName());
	...
	LogServiceClientSoapDB() {

	}

	// TIPO = REQUEST|RESPONSE VERSO = CLIENT|SERVER
	LogServiceClientSoapDB(String tipo, String verso, String nomeServizio, String mitt, String codiceFiscaleReq) {
		this.tipo = tipo;
		this.verso = verso;
	...
	// TIPO = REQUEST|RESPONSE VERSO = CLIENT|SERVER
	LogServiceClientSoapDB(String tipo, String verso, String codEsito) {
		this.tipo = tipo;
		this.verso = verso;
		this.esito = codEsito;
	}
	...
	// TIPO = REQUEST|RESPONSE VERSO = CLIENT|SERVER
	LogServiceClientSoapDB(String tipo, String verso, String codEsito, String codiceFiscaleReq) {
		this.tipo = tipo;
		this.verso = verso;
		this.esito = codEsito;
		this.codiceFiscaleReq = codiceFiscaleReq;
	...
	public void invoke(MessageContext msgContext) throws AxisFault {
		log.debug("LogServiceClientSoapDB::inizio");
		...
		try {
			...
			//////////////////////////////////////////
			// INSERIMENTO NELLA TS_WS_TRACCIAMENTO //
			//////////////////////////////////////////
			...
			if (this.tipo.equalsIgnoreCase("REQUEST")) {
				...
				/////////////////////////////////////////
				// INSERIMENTO NELLA TS_WS_LOG_SERVIZI //
				/////////////////////////////////////////
				...
				if (rs1.next()) {
					...
					msgContext.setProperty("KEY_WS_LOG_SERVIZI", prgWsServiziLog);
				} else {
					throw new Exception("Errore nella sequence");
				}
			...
			} else if (this.tipo.equalsIgnoreCase("RESPONSE")) {

				///////////////////////////////////////////
				// AGGIORNAMENTO DELLA TS_WS_LOG_SERVIZI //
				///////////////////////////////////////////

				if (msgContext.getProperty("KEY_WS_LOG_SERVIZI") != null) {

					prgWsServiziLog = (BigDecimal) msgContext.getProperty("KEY_WS_LOG_SERVIZI");

					if (codiceFiscaleReq != null) {
						String SQL_Service = UPDATE_SQL_SERVICE_RESP_WITH_CF.replaceFirst("%TABELLA%", TABELLA_SERVIZI);
						PreparedStatement ps1 = conn.prepareStatement(SQL_Service);
						ps1.setString(1, esito);
						ps1.setBigDecimal(2, prgWsTracciamento);
						ps1.setString(3, codiceFiscaleReq);
						ps1.setBigDecimal(4, prgWsServiziLog);
						ps1.execute();
						ps1.close();
					} else {
			...
			log.error("LogServiceClientSoapDB::Si e' verificato un errore: " + toString(), e);
