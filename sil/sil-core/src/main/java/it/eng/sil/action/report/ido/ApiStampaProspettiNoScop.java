...
public class ApiStampaProspettiNoScop {
	...
	public ApiStampaProspettiNoScop(SourceBean conf) {
		this.config = conf;
	}
	...
	private void testata(Area area) throws Exception {
		...
		p.setHeight(0);
		tp = p.addTextPart(
				"Lista delle aziende che nell'anno " + anno + " non presentano scoperture. " + codMonoStatoProspetto);
		s.setStyle(tp);
		tp.setFontSize(12);
		tp.setFontName("Times New Roman");
		tp.setBold(true);
