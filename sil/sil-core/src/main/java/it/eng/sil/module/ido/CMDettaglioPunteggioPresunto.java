 ...
 */
public class CMDettaglioPunteggioPresunto extends AbstractSimpleModule {

	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger
			.getLogger(CMDettaglioPunteggioPresunto.class.getName());
		...
		try {
			...
			if (("2").equalsIgnoreCase(prgTipoRosa)) {
				...
				if (!("12").equalsIgnoreCase(prgTipoIncrocio)) {
					...
					/*
					 * verifica versione graduatoria in ts_generale presenza parametro CODMONOTIPOGRAD 1: calcolo
					 * punteggio normale 2: calcolo punteggio con locomozione e patente
					 */
					...
					// nuova versione: calcola i mesi di anzianità dalla data di inizio pubblicazione della richiesta
				...
				if (numMesiAnzianita >= 0) {
					...
					// aggiorno a null il punteggio (non calcolabile per l'assenza dei mesi di anzianità)
					SourceBean statement = (SourceBean) getConfig().getAttribute("CM_UPDATE_PUNTEGGIO_PRES");
					esito = (Boolean) transExec.executeQuery(getRequestContainer(), getResponseContainer(), statement,
							"UPDATE");
	...
	private void calcolaPunteggio(SourceBean request, SourceBean response, int numMesiAnzianita, Object prgNominativo,
		...
		try {
			...
			if (numMesiAnzianita >= 0) {
				...
				row.setAttribute("flgpatentePres", flgpatente);
				row.setAttribute("codgradocapacitalocPres", codgradocapacitaloc);
