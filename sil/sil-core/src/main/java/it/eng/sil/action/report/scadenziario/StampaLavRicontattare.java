...
public class StampaLavRicontattare extends AbstractSimpleReport {
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger(StampaLavRicontattare.class.getName());
	...
	public void service(SourceBean request, SourceBean response) {
		...
		if (apriFile != null && apriFile.equalsIgnoreCase("true")) {
			...
			if (tipoFile != null)
				setStrNomeDoc("LavRicontattare." + tipoFile);
			else
				setStrNomeDoc("LavRicontattare.pdf");
			setStrDescrizione("Lavoratori da Ricontattare");
			...
			try {
				...
				if ((salva != null) && salva.equalsIgnoreCase("true")) {
					eng = executeQueriesLavRicont(request, response);
	...
	com.inet.report.Engine executeQueriesLavRicont(SourceBean request, SourceBean response) throws Exception {
		String tipoFile = (String) request.getAttribute("tipoFile");
		SourceBean listaLavRicont = doDynamicSelect(request, response);

		Vector vLavRicont = listaLavRicont.getAttributeAsVector("ROW");

		ApiLavRicontattare report = new ApiLavRicontattare((SourceBean) getConfig());
		...
		report.setElencolav(vLavRicont);
		report.start();
