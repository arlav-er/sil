...
public class TrasmettiPattoSoapBindingImpl implements it.eng.sil.coop.webservices.pattoonline.TrasmettiPatto {
	...
	public it.eng.sil.coop.webservices.pattoonline.EsitoType aggiornaPatto(
		...
		try {
			...
			boolean inputXMLIsValid = XmlUtils.isXmlValid(inputXML, aggiornaPattoOnline_schema);
			if (!inputXMLIsValid) {
			...
			if (result == 0) {
				_logger.info(
						"Aggiornamento dati accettazione patto on line in  am_patto_lavoratore avvenuto con successo");
				trans.commitTransaction();
				esito.setEsito(EsitoTypeEsito.OK);
				esito.setDescrizione("Dati accettazione patto registrati con successo");
			} else {
				_logger.info("Aggiornamento dati accettazione patto on line in  am_patto_lavoratore non riuscito");
