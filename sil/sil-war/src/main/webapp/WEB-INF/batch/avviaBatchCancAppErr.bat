:: BATCH DEPRECATO, DA ELIMINARE. IN SOSTITUZIONE REALIZZATO TIMER CancellaMovAppErroreTimer
@ECHO OFF
...
echo =============================================================== >> %log_err%
echo %timestamp% - ========= Avvio batch Cancellazione Movimenti da appoggio la cui validazione ha restituito un dato errore ========= >> %log_err%
...
SET parameters=-DAF_ROOT_PATH=..\.. -DAF_CONFIG_FILE=/WEB-INF/conf/master_batch.xml -DCONTEXT_NAME=%context_name%
SET class_name=it.eng.sil.util.batch.BatchCancellaMovAppErrore
SET batch_launcher=it.eng.sil.util.batch.BatchLauncher

CALL ..\conf\custom\%hostname%_%context_name%\global_properties.bat

rem SET parameters=%parameters%

@ECHO ON

%JAVA_HOME%\bin\java -cp .;%libs% %parameters% %class_name% 00/00/0000 00/00/0000 X X X X 50133 00/00/0000 00/00/0000 X %batch_user%
