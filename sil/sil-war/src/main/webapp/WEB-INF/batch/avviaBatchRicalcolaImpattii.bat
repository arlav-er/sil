:: BATCH DEPRECATO, DA ELIMINARE. IN SOSTITUZIONE REALIZZATO TIMER RicalcolaImpattiTimer
@ECHO OFF
...
echo =============================================================== >> %log_err%
echo %timestamp% - ========= Avvio batch Ricalcola Impatti ========= >> %log_err%
...
SET class_name=it.eng.sil.util.batch.BatchRicalcolaImpatti
SET batch_launcher=it.eng.sil.util.batch.BatchLauncher

CALL ..\conf\custom\%hostname%_%context_name%\ricalcola_impatti_properties.bat

SET parameters=%parameters% -D_ENCRYPTER_KEY_=%_ENCRYPTER_KEY_%

@ECHO ON

%JAVA_HOME%\bin\java -cp .;%libs% %parameters% %class_name% %batch_user%
