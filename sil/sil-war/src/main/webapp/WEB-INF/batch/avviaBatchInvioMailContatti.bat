:: BATCH DEPRECATO, DA ELIMINARE. IN SOSTITUZIONE REALIZZATO TIMER InvioMailContattiTimer
@ECHO OFF
...
echo =============================================================== >> %log_err%
echo %timestamp% - ========= Avvio batch Invio Mail Contatti  ========= >> %log_err%
...
SET libs=%libs%;"%JBOSS_SERVER_LIB%\mail.jar"

REM Parametri necessari per il lancio del batch

SET parameters=-DAF_ROOT_PATH=..\.. -DAF_CONFIG_FILE=/WEB-INF/conf/master_batch_invio_mail_contatti.xml -DCONTEXT_NAME=%context_name%
SET class_name=it.eng.sil.util.batch.BatchInvioMailContatti
...
%JAVA_HOME%\bin\java -cp .;%libs% %parameters% %class_name% 365  
