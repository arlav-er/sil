...
function scriviFrame() {
    ...
    actionFrame.open();
    actionFrame.write('<%=getFormToAction()%>');
    actionFrame.close();
}
...
function indietro(){
	window.top.main.location="AdapterHTTP?<%= JSPReportUtil.getQueryString(request)%>";	
	window.top.footer.document.getElementById("sezPulsanti").style.display="none";
}
...
String makeInput() {
    return "<input type=\"hidden\" name=\"PAGE\" value=\"REPORTSTAMPAPAGE\">";
}
%>
