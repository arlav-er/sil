...
<%
	...
	boolean isFromStorico = false;
	if(serviceRequest.containsAttribute("IS_STORICO")){
		isFromStorico = true;
		paginaIndietro = "StoricoNotificheRDCPage";
	}
	...
	String strDataRicezioneSIL = null;
	String fonte = null;
	String strDataPassaggio = null;
	
	String statoDescrFullInps = null;
	String statoDescrFullRuolo = null;
	
	String codTipoDomanda = null;
	String tipoDomandaDescr = null;
	String tipoDomandaDescrFull = null;
	...
	titolo = "Dettaglio Notifica RDC";
	 if(serviceResponse.containsAttribute("M_DettaglioNotificaRDC")){
		...
		if(row!=null){
			...
			strDescrStatoInps = SourceBeanUtils.getAttrStrNotNull(row, "strdescrstatoinps");
			statoDescrFullInps = statoDomInps + " - " + strDescrStatoInps;
			...
			strDataRicezioneSIL = SourceBeanUtils.getAttrStrNotNull(row, "STRDATARICEZIONESIL");
			fonte =  SourceBeanUtils.getAttrStrNotNull(row, "STRFONTE");
			strDataPassaggio = SourceBeanUtils.getAttrStrNotNull(row, "STRDATAPASSAGGIO");
			statoDescrFullRuolo = codRuolo + " - " + descrCodMonoRuolo;
			codTipoDomanda = SourceBeanUtils.getAttrStrNotNull(row, "CODTIPODOMANDA");
			strDataVariazioneStato = SourceBeanUtils.getAttrStrNotNull(row, "STRDATAVARIAZIONESTATO");
		}
	}		
...
<script language="Javascript">
  ...
  function tornaLista() {
	...
	function goStoricoNotifiche(){
		var urlpage="AdapterHTTP?";
  		var nomePagina = "StoricoNotificheRDCPage";
   		urlpage+="CDNFUNZIONE=<%=cdnfunzioneStr%>&PAGE=";
   		urlpage += nomePagina;
		urlpage +="&CFRICH=<%=strCodiceFiscaleRich%>";
  		window.open(urlpage,'Storico Notifiche RDC','toolbar=NO,statusbar=YES,width=900,height=500,top=50,left=100,scrollbars=YES,resizable=YES');
	}
		...
		<tr>
			...
			<td class="campo">
			 <af:textBox name="domandaInps" type="text"
							...
							value="<%= statoDescrFullInps %>"
							size="60" maxlength="101"
							classNameBase="input"
							readonly="true"/>
<%-- 				/>&nbsp;-&nbsp; --%>
<%-- 				 <af:textBox name="strDescrStatoInps" type="text" --%>
<%-- 							 title="Descr Domanda INPS" --%>
<%-- 							value="<%= strDescrStatoInps %>" --%>
<%-- 							size="50"   --%>
<%-- 							classNameBase="input" --%>
<%-- 							readonly="true" --%>
<%-- 				/> --%>
		...
		<tr>
			...
			<td class="campo">
			 <af:textBox name="dataRicezioneSIL" type="text"
							title="Data Ricezione SIL"
							value="<%= strDataRicezioneSIL %>"
							size="15" maxlength="101"
							classNameBase="input"
							readonly="true"
				/>
		...
		<tr>
			...
			<td class="campo">
			 <af:textBox name="dataPassaggio" type="text"
							title="Data passaggio da patto per inclusione al patto per il lavoro"
		...
		<%if(!isFromStorico){ %>
			<tr>
				<td colspan="2" align="center">
					<input class="pulsanti" type="button" name="btnGetStoricoNotificheRDC" onclick="goStoricoNotifiche();" value="Storico Notifiche RDC"/>
				</td>
			</tr>
		<%} %>
 			<tr>
				<td colspan="2" align="center">
				<%if(!fromCruscotto || isFromStorico){ %>
