...
<p>
	<font color="green">
		<af:showMessages prefix="M_SalvaEventoTDA"/>
		<af:showMessages prefix="M_AggiornaEventoTDA"/>
		<af:showMessages prefix="M_DeleteEventiTDA"/>
  	</font>
...
<p>
	<af:list moduleName="M_ListaEventiTDA" canDelete="<%= String.valueOf(canDelete ? 1 : 0) %>"/>
</p>

<center>
<af:form method="POST" action="AdapterHTTP">
	<input type="hidden" name="PAGE" value="TDADettaglioEventoPage"/>
	<input type="hidden" name="CDNFUNZIONE" value="<%=_funzione%>"/>
	<input type="hidden" name="PRGVOUCHER" value="<%=prgVoucher%>"/>
	<%if (canInsert) {%>
		<input class="pulsanti" type="submit" name="inserisci" value="Nuovo Evento"/>
	<%}%>
