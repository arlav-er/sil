...
PageAttribs attrGraduatorie = new PageAttribs(user, "CMGestGraduatoriePage");
...
<af:list moduleName="CMElencoGraduatorieModule"/>
...
<tr>
  <td align="center">
  	<% String txtReturn = "PAGE=IdoTestataRichiestaPage&PRGRICHIESTAAZ="+prgRichiestaAz+"&CDNFUNZIONE="+_cdnFunzione+"&PRGAZIENDA="+prgAzienda+"&PRGUNITA="+prgUnita ;%>
...
%>
			  ...
			  <td align="right">
			      <input type="button" name="Documenti associati" value="Documenti associati" class="pulsante"
			             onClick="docAssociatiAzienda('<%= prgAzienda %>', '<%= prgUnita %>', 'CMGestGraduatoriePage', '<%=_cdnFunzione%>', null, '<%=prgOrig%>')">
			  </td>	
<%
}
%>
...
%>
			  <td align="right" width="39%">
			  	  <% 
			  	  String txtInfoStoriche = "PAGE=CMStoricoGraduatoriePage&cdnFunzione="+_cdnFunzione+"&prgAzienda="+prgAzienda+"&prgRichiestaAZ="+prgRichiestaAz+"&prgUnita="+prgUnita+"&PRGORIG="+prgRichiestaAz;
			  	  %>
