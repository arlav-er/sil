...
<%
	...
	boolean isAtecoEdile = false;
	boolean isCcnlEdile = false;
	
	SourceBean isAtecoEdileSB = (SourceBean) serviceResponse.getAttribute("M_IsEdileAtecoUnitaAz");
	if (isAtecoEdileSB.getAttribute("ROWS.ROW.FLGEDILE") != null && "S".equals(isAtecoEdileSB.getAttribute("ROWS.ROW.FLGEDILE"))) {
		isAtecoEdile = true;
	}
	
	SourceBean isCcnlEdileSB = (SourceBean) serviceResponse.getAttribute("M_IsEdileCcnlUnitaAz");
	if (isCcnlEdileSB.getAttribute("ROWS.ROW.FLGEDILE") != null && "S".equals(isCcnlEdileSB.getAttribute("ROWS.ROW.FLGEDILE"))) {
		isCcnlEdile = true;
	}
	...
	SourceBean contInvioClicLavoro = (SourceBean) serviceResponse.getAttribute("M_GetDatiBlen.ROWS.ROW");
...
%>
		...
		<script type="text/javascript" language="Javascript">   		
			...
			function SubmitSeConfermaInvio(autoConfirm) {
				...
				if(dataInv < dataRichiesta){
					//if (!autoConfirm) alert("La data di invio a BLEN deve essere compresa nel periodo di validità della richiesta di personale.");
					alert("La data di invio a BLEN deve essere compresa nel periodo di validità della richiesta di personale.");
					return false;
				}
				...
				if(dataScad > dataScadenzaRich){
					if (!autoConfirm) alert("Il periodo di validità su BLEN deve essere compreso nel periodo di validità della richiesta di personale.");
					return false;
				}
				*/
				...
				//Controllo codAteco unità az edile
				var isAtecoEdile = document.Frm1.isAtecoEdile.value;
				var isCcnlEdile = document.Frm1.isCcnlEdile.value;
				if (isAtecoEdile == "false" && isCcnlEdile == "false") {
					alert("Il codice ATECO e/o il codice CCNL dell'azienda non rientrano tra i codici edili consentiti per l'invio.");
					return false;
				}
				...
				if (mansioni != "1"){
					//if (!autoConfirm) alert("Per poter completare l'invio della vacancy è necessario inserire una mansione del Profilo n. 1 che dispone della codifica ministeriale e selezionare per l'invio a BLEN.");
				...
				if (document.Frm1.desmansione.value == null || document.Frm1.desmansione.value.length == 0){
					//if (!autoConfirm) alert("Non esiste una descrizione della qualifica professionale offerta, ovvero dell'unica mansione del Profilo n. 1 selezionata per l'invio a BLEN.");
					alert("Non esiste una descrizione della qualifica professionale offerta, ovvero dell'unica mansione del Profilo n. 1 selezionata per l'invio a BLEN.");
					return false;
				}
				...
				if (comuni != "1"){
					//if (!autoConfirm) alert("Per poter completare l'invio della vacancy è necessario inserire un territorio di tipo comune e selezionare per l'invio a BLEN.");
					alert("Per poter completare l'invio della vacancy è necessario inserire un territorio di tipo comune e selezionare per l'invio a BLEN.");
					return false;
				}
				...
				if (codEvasione !== "DFD" && codEvasione !== "DPR" && codEvasione !== "DFA" && codEvasione !== "DRA" || statoPubblic != "In pubblicazione"){
					//if (!autoConfirm) alert("Per poter completare l'invio della vacancy a BLEN è necessario che lo stato della vacancy sia 'In pubblicazione' e la Modalità di evasione sia 'Pubblicazione Palese' o 'Pubblicazione Palese / Preselezione' o 'Pubblicazione Anonima' oppure 'Pubblicazione Anonima / Preselezione'.");
					alert("Per poter completare l'invio della vacancy a BLEN è necessario che lo stato della vacancy sia 'In pubblicazione' e la Modalità di evasione sia 'Pubblicazione Palese' o 'Pubblicazione Palese / Preselezione' o 'Pubblicazione Anonima' oppure 'Pubblicazione Anonima / Preselezione'.");
					return false;
				}
				...
				if (codRapportoLav == null || codRapportoLav === ""){
					//if (!autoConfirm) alert("Per poter completare l'invio della vacancy a BLEN è necessario selezionare un Rapporto di Lavoro.");
				...
				return confirm("Sei sicuro di voler procedere all'invio della richiesta di personale su BLEN?");
			}
			...
			function apriAnteprimaInvio() {
		    	...
		    	var opened;
		        var f = "AdapterHTTP?PAGE=IdoInvioBlenAnteprimaPage&prgRichiestaAz="+prgRichiestaAz
	...
	<center>
      <font color="green">
        ...
        <af:showMessages prefix="M_InsertBlen" />        
      </font>
      <font color="red">
        <af:showErrors/>
      </font>
	...
	<p align="center">						
		...
		<af:form name="Frm1" action="AdapterHTTP" method="post" onSubmit="SubmitSeConfermaInvio()">
			...
			<input type="hidden" name="isAtecoEdile" value="<%=isAtecoEdile%>">
			<input type="hidden" name="isCcnlEdile" value="<%=isCcnlEdile%>">
			...
			%>
			<br/><b>
			Sincronizza l'offerta di lavoro con BLEN
			</b><br/><br/>
			<table class="main">
				<%--
				<tr>
