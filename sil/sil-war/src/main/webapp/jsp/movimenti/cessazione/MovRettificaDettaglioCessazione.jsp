<!-- Rettifica DETTAGLIO CESSAZIONE -->
...
<%      
	...
	PageAttribs attributi = new PageAttribs(user, "MovDettaglioCessazioneRettificaPage");
...
<%
  ...
  String codsoggetto = "";
	...
	if (datProtV.equals("")) datProtV = StringUtils.getAttributeStrNotNull(serviceRequest, "dataProt2");
	if (oraProtV.equals("")) oraProtV = StringUtils.getAttributeStrNotNull(serviceRequest, "oraProt2");
	if (strNumAnnoProt.equals("")) strNumAnnoProt = StringUtils.getAttributeStrNotNull(serviceRequest, "numAnnoProt2");
...
<html>
  ...
  <body class="gestione" onload="inizializzaCollegati('<%=prgMovimentoPrec%>', '<%=prgMovimentoSucc%>');rinfresca();calcolaDiffGiorni(document.Frm1.datInizioMov, document.Frm1.NUMGGTRAMOVCOMUNICAZIONE,varRange,oggettoInSessione);notificaCambioStatoOcc();gestVisualGiorniRitardo();impostaGrado();">
...
<% if (!prgAzienda.equals("") && !prgUnita.equals("") && !cdnLavoratore.equals("")) { testataAz.show(out); }%>  
  <%@ include file="../common/include/GestioneRisultati.inc" %>
    ...
    <center>
      <af:form name="Frm1" method="POST" action="AdapterHTTP" onSubmit="checkDatInizioMovImpattiCessazione(document.Frm1.datInizioMov, document.Frm1.DATINIZIOMOVPREC) && controlloLavAutonomo()">
