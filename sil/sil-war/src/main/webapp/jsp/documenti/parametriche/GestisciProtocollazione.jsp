...
<%
//si decide quale page da chiamare quando si clicca OK
String pageToProfile = (String)serviceRequest.getAttribute("PAGE_DA_CHIAMARE");
...
<% 
	...
	String parametri     = (String) serviceRequest.getAttribute("parametri");
   	...
   	//Ottengo la data e ora corrente
    ...
    String oraProt = ora + ":" + minuti;
   	...
   	//Carico le info del tipo documento che voglio protocollare
   	String nomeDoc = null;
   	String docRif  = null; 
...
//   	String codTipoDocumento = null;
   	...
   	rows = serviceResponse.getAttributeAsVector("M_GetTemplateByTipoDoc.ROWS.ROW");
   	if(rows != null && !rows.isEmpty()){ 
		SourceBean row = (SourceBean) rows.elementAt(0);
	//	codTipoDocumento  = (String) row.getAttribute("CODTIPODOCUMENTO");
		BigDecimal prg  = SourceBeanUtils.getAttrBigDecimal(row, "PRGTEMPLATESTAMPA", null);
		prgTemplateStampa = prg.toString();
  	}
   	...
   	if(rows != null && !rows.isEmpty()){ 
		SourceBean row = (SourceBean) rows.elementAt(0);
     	...
     	numKloProt = (BigDecimal) row.getAttribute("NUMKLOPROTOCOLLO");
   	}
  	...
  	BigDecimal numProtEsist    = null;
   	...
   	// Se esiste un documento di quel tipo devo consentire di visualizzarlo
...
function apriStampa(tipoDoc,prgTemplateStampa) {
   var urlpage;
   var myform = window.document.forms['form1'];
   ...
   //valorizzare il PRGTEMPLATESTAMPA nell'opener
   
  if (apri&&!PR){
   	window.opener.document.frmTemplate.GENERASTAMPA.value = 'VISUALIZZA';
   	window.opener.document.frmTemplate.TIPODOC.value = tipoDoc;
   	window.opener.document.frmTemplate.PRGTEMPLATESTAMPA.value = prgTemplateStampa;
...
<tr><td colspan="2" align="center">
    <input class="pulsante" name="prosegui" type="button" value="  OK   "  onclick="apriStampa('<%=tipoDoc%>','<%=prgTemplateStampa%>')"/>&nbsp;&nbsp;&nbsp;
