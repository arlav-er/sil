...
<% 
   ...
   if (tipoDoc != null && tipoDoc.equalsIgnoreCase("CIGDOM")) {
	...
	if (configurazione.equals("1")) {
		nomeDoc = "ISCRIZIONE LISTE SPECIALI";
		docRif = "STAMPA ISCRIZIONE";
	}
   }
   ...
   boolean prObbligatoria  = (tipoDoc.equalsIgnoreCase("IM") || tipoDoc.equalsIgnoreCase("PT297") || tipoDoc.equalsIgnoreCase("ACLA") ||
                              tipoDoc.equalsIgnoreCase("SA") || tipoDoc.equalsIgnoreCase("SAP") || tipoDoc.equalsIgnoreCase("SAPM"))
                              || tipoDoc.equalsIgnoreCase("TRCPI")  || tipoDoc.equalsIgnoreCase("TRDOC")
                              || tipoDoc.equals("DICH") || tipoDoc.equals("ATT") 
                              || tipoDoc.equals("ALMOBO") || tipoDoc.equals("REI") ? true : false;
   ...
   if (serviceResponse.containsAttribute("M_ExistDocumentPerChiave")) {
	   rows = serviceResponse.getAttributeAsVector("M_ExistDocumentPerChiave.ROWS.ROW");	   
   }
   else {
   ...
   { SourceBean row = (SourceBean) rows.elementAt(0);
     ...
     fileEsistente = true;
     if(isRistampa){
    	 numProt          = numProtEsist;
         numAnnoProt      = annoP;
         strAnnoProt = annoProtEsist;
         strNumProt =numProtEsist!=null ? numProtEsist.toString() : ""; numProtEsist.toString();
         dataProt = SourceBeanUtils.getAttrStr(row, "dataProt", "");
         oraProt = SourceBeanUtils.getAttrStr(row, "oraProt", "");
     }
   }
	...
	if (tipoDoc.equalsIgnoreCase("NULOST") ) {
		valoreDB = "Salva stampa come documento";
	}
	
	InfoRegioneSingleton regione = InfoRegioneSingleton.getInstance();  
	String regionePolo = regione.getCodice(); 

	boolean canViewFileEsistente = !((tipoDoc.equalsIgnoreCase("PT297") || tipoDoc.equalsIgnoreCase("ACLA")) && it.eng.sil.module.movimenti.constant.Properties.UMB.equalsIgnoreCase(regionePolo));
...
function apriFileEsistente(PR) {
  ...
  if (myform.apriFileBlob.checked) {
    ...
    if(PR) 
    <%if (!tipoDoc.equalsIgnoreCase("NULOST") && !tipoDoc.equalsIgnoreCase(Properties.TIPO_DOC_TDA_ATTIVAZIONE) && 
    	  !tipoDoc.equalsIgnoreCase(Properties.TIPO_DOC_TDA_PARTECIPAZIONE) ) {%> 
    	myform.protocolla.checked= true;
...
<tr>
    ...
    <td>
		...
		<%if ((tipoDoc.equals("STMOV")) || (tipoDoc.equals("PRM")) || (tipoDoc.equals("RAP")) || (tipoDoc.equals("RGR")) || (tipoDoc.equals("PUB")) 
            	...
            	|| tipoDoc.equals("NOF") || tipoDoc.equals("STRA") || tipoDoc.equals(Properties.TIPO_DOC_TDA_ATTIVAZIONE) 
            	|| tipoDoc.equals(Properties.TIPO_DOC_TDA_PARTECIPAZIONE)) {
...
<tr><td><img name="DBImg" alt="Salva sul DataBase" src="../../img/DB_img.gif"></td>
    <td><table cellpadding="0" cellspacing="0" border="0" width="100%">
        <tr><td>
          ...
          <%if (tipoDoc != null && (tipoDoc.equalsIgnoreCase(Properties.TIPO_DOC_TDA_ATTIVAZIONE) || tipoDoc.equalsIgnoreCase(Properties.TIPO_DOC_TDA_PARTECIPAZIONE)) && 
                fileEsistente){%>
          ...
          <%} else {
	          ...
	          || (tipoDoc.equals("PUB_G")) || tipoDoc.equals("SLCOOP") || tipoDoc.equals("PROF_DIA_RURER") || tipoDoc.equals("STRA")){%>        
	          ...
	          <%
	            ...
	            if(prObbligatoria || forzaSalvaDB){%>
	                <input name="salvaDB" type="checkbox" checked disabled="disbled"/>Salvare nel DataBase
	             <%} else {%>
	                <input name="salvaDB" type="checkbox"/><%=valoreDB%>
	             <%}
	          }
           } %>
            </td>
...
<%
  if (fileEsistente && canViewFileEsistente) {%>
  <tr><td><br/></td></tr>
