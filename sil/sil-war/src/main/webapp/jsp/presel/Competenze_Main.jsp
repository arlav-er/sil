...
function Insert() {
  document.Frm1.MODULE.value = "M_InsertCompetenza";
  if(controllaFunzTL())  {
...
function Update()
  {
    ...
    if (datiOk) {
      document.Frm1.MODULE.value = "M_SaveCompetenza";
...
<body class="gestione" onload="rinfresca()">
  ...
  %>
    ...
    <af:form method="POST" action="AdapterHTTP" name="Frm1">
      ...
      <input type="hidden" name="MODULE" value="M_InsertCompetenza"/>
      ...
      <center>
        <font color="green">
          <af:showMessages prefix="M_InsertCompetenza"/>
      ...
      %>
          <p align="center">
            <input type="button" class="pulsanti" onClick="apriNuovoDivLayer(<%=nuovo%>,'divLayerDett','<%=url_nuovo%>');document.location='#aLayerIns';" value="Nuova competenza"/>        
          </p>
      <%}
        ...
        <br>
          <table class="main">
              <tr valign="top">
                ...
                <td class="campo">
                  <% if (nuovo) {%>
                    <af:comboBox title="Tipo di competenza" name="codTipoCompetenza" moduleName="M_GETTIPICOMPETENZA" addBlank="true" onChange="javascript:caricaCompetenze(Frm1.codTipoCompetenza.value,true)" required="true" /> 
                  <%}
                      else {%>
