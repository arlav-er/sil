...
<%
  ...
  int _funzione = 1;
  //Integer.parseInt((String) serviceRequest.getAttribute("CDNFUNZIONE"));
...
<%--
  ...
  //Servono per gestire il layout
	...
	SourceBean esisteCandidaturaValidaSb = (SourceBean) serviceResponse.getAttribute("M_CL_ESISTE_CANDIDATURA_VALIDA.ROWS.ROW");
	boolean esisteCandidaturaValida = !(esisteCandidaturaValidaSb == null || esisteCandidaturaValidaSb.getContainedAttributes().isEmpty());
	SourceBean candidatureRows= (SourceBean) serviceResponse.getAttribute("M_CL_LIST_INVII_CANDIDATURE.ROWS");
...
<head>
<title>Lista delle candidature</title>
...
<body class="gestione">
  ...
  %> 	
  <%--
		if (esisteCandidaturaValida) {
			EMFErrorHandler engErrorHandler = responseContainer
					.getErrorHandler();
			Vector vParam = new Vector();
			vParam.add(": Esiste una candidatura valida");
			EMFUserError userError = new EMFUserError(
					EMFErrorSeverity.WARNING,
					MessageCodes.ClicLavoro.LAVORATORE_NON_CANDIDABILE,
					vParam,
					"");
	...
	<center>
		<p class="titolo">Lista delle candidature</p>
	</center>

	<div align="center">
		<af:list moduleName="M_CL_LIST_INVII_CANDIDATURE"
			canInsert="<%=canModify ? \"1\" : \"0\"%>" skipNavigationButton="1"
			jsSelect="dettaglioInvioCliclavoro" />
		<%
	 if (isButtVisible){
	 %>
	...
	<p />
	 ...
	 %>
		...
		<af:form name="FormNuovaCandidatura" action="AdapterHTTP" method="post">
			...
			<input type="hidden" name="PAGE" value="CL_INVIO_CANDIDATURA">
