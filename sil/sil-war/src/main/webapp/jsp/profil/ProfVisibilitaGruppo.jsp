...
<%@ page import="
	...
	" %>
<%!  
	static org.apache.log4j.Logger _logger = org.apache.log4j.Logger.getLogger("it.eng.sil._jsp.ProfVisibilitaGruppo.jsp");
%>
...
<af:form action="AdapterHTTP" method="POST" dontValidate="true">
   ...
   <af:textBox type="hidden" name="CDNGRUPPO" value="<%=cdnGruppoStr%>" />
   <af:textBox type="hidden" name="PAGE" value="ProfVisibilitaXMLSalvaPage" />
...
<%
	StringBuffer xml = (StringBuffer) serviceResponse.getAttribute("PROFVISIBILITAXML.BUFXML");
	...
	File.separator + "profil" + File.separator + "visibilitaXML.xsl";
	...
	transformer.transform(new StreamSource(new StringReader(xml.toString())), new StreamResult(result));
		...
		_logger.fatal( 
			"trasformazione XML-HTML per filtri di visibilita", ex);	
	}
	out.println(" <!--inizio--> ");
