...
<body class="gestione" onload="rinfresca();caricaArea('2')">
	...
	//Linguette l = new Linguette(user, _funzione, "CMInformaPage", new BigDecimal(cdnLavoratore));
	//Linguette_Parametro l = new Linguette_Parametro(user, _funzione, "CMInformaPage", cdnLavoratore, "1",true);	
	LinguetteConfigurazioneRegione l = new LinguetteConfigurazioneRegione (user, _funzione, "CMInformaPage" , cdnLavoratore , "1", true , "LNDGNFNZ");
	...
	<af:form method="POST" action="AdapterHTTP" name="Frm1" onSubmit="">
		...
		<input type="hidden" name="PAGE" value="CMInformaPage" />		
...
<%
		...
		if(readCodiciScaduti) {
			rows= serviceResponse.getAttributeAsVector("M_Load_Descr_Cap_Informa_Scad.ROWS.ROW");
		} else {
			rows= serviceResponse.getAttributeAsVector("M_Load_Descr_Cap_Informa.ROWS.ROW");
		}
%>
