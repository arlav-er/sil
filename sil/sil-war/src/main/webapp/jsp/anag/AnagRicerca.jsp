...
<%
 // NOTE: Attributi della pagina (pulsanti e link) 
  ...
  String msgCriteriRicerca = "Per effettuare una ricerca inserire almeno i primi sei caratteri del codice fiscale o almeno i primi due caratteri del cognome";
  boolean utenteConvenzione = false; 
 
  SourceBean profiloUtente = (SourceBean) serviceResponse.getAttribute("ProfDettaglioUtenteSession.rows.row");
  SourceBean sbConfigNoce = (SourceBean) serviceResponse.getAttribute("M_ConfigConvezioneNoce.rows.row");
  
  String tipoGruppoCollegato = user.getCodTipo();
  
  if (profiloUtente != null && profiloUtente.containsAttribute("flgUtConvenzione") && profiloUtente.getAttribute("flgUtConvenzione").toString().equalsIgnoreCase("S") &&
     sbConfigNoce != null && sbConfigNoce.containsAttribute("strvalore") && 
     sbConfigNoce.getAttribute("strvalore").toString().equalsIgnoreCase(Properties.CUSTOM_CONFIG)) {
	 utenteConvenzione = true;
	 msgCriteriRicerca = "Per effettuare una ricerca inserire tutti i caratteri del codice fiscale";
  }
...
<html>
    ...
    <af:linkScript path="../../js/" />
     ...
     <% 
      ...
      %>
      <!--
      var utenteConvenzione = '<%=utenteConvenzione%>';
      ...
      function checkCampiObbligatori()
      {
        if (tipoGruppoUtCollegato == 'S') {
			...
			if (utenteConvenzione == 'false') {
				...
				if (document.Frm1.strCodiceFiscale.value != null && document.Frm1.strCodiceFiscale.value.length == 16) {
					return true;
				}
				else {
					alert("Inserire tutti i caratteri del codice fiscale");
			        return false;
				}
			}
        }
  ...
  <p class="titolo">Ricerca sull'anagrafica Lavoratori</p>
    <%out.print(htmlStreamTop);%>
      ...
      <table class="main">
        <tr><td colspan="2"/><br/><%=msgCriteriRicerca%></td></tr>
        
        <%if (utenteConvenzione) {%>
        	 ...
        	 <tr>
	          ...
	          <td class="campo">
	       	...
	       	<input type="hidden" name="tipoRicerca" value="esatta"/>
	       	<input type="hidden" name="strCognome" value=""/>
	       	<input type="hidden" name="strNome" value=""/>
	       	<input type="hidden" name="datnasc" value=""/>
	       	<input type="hidden" name="codComNas" value=""/>
	       	<input type="hidden" name="strComNas" value=""/>
        <%}
        else {%>
	        <tr><td colspan="2"/>&nbsp;</td></tr>
	        <tr>
        ...
        <input type="hidden" name="PAGE" value="AnagRicercaPage"/>
