...
<body class="gestione" onload="rinfresca()">
	...
	//Linguette l = new Linguette(user, _funzione, "CMMovPage", new BigDecimal(cdnLavoratore));
	//Linguette_Parametro l = new Linguette_Parametro(user, _funzione, "CMMovPage", cdnLavoratore, "1", true);
	LinguetteConfigurazioneRegione l = new LinguetteConfigurazioneRegione (user, _funzione, "CMMovPage" , cdnLavoratore , "1", true , "LNDGNFNZ");
	...
	<af:form method="POST" action="AdapterHTTP" name="Frm1" onSubmit="">
		...
		<input type="hidden" name="PAGE" value="CMMovPage" />		
...
<%
		...
		if(readCodiciScaduti) {
			rows= serviceResponse.getAttributeAsVector("M_Load_Descr_Cap_Mov_Scad.ROWS.ROW");
		} else {
			rows= serviceResponse.getAttributeAsVector("M_Load_Descr_Cap_Mov.ROWS.ROW");
		}
%>
			...
			<td class="campo" >
				<%capCogn = "capCogn_" + indice;
				  ...
				  <%} else if(codMonoTipoGrado.equals("R")){%>
				  	<af:comboBox name="<%=capCogn%>" title="Capacita cogn" selectedValue="<%=codCapCogn%>" required="false"
		      	  		 		 moduleName="M_ComboTipoImpegni" disabled="<%= String.valueOf(readCodiciScaduti) %>"
			      		 		 classNameBase="input" onChange="fieldChanged()" addBlank="true"/>
