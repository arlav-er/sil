...
<body class="gestione" onload="rinfresca();caricaArea('4')">
	...
	//Linguette l = new Linguette(user, _funzione, "CMCapMenRelPage", new BigDecimal(cdnLavoratore));
	//Linguette_Parametro l = new Linguette_Parametro(user, _funzione, "CMCapMenRelPage", cdnLavoratore, "1", true);
	LinguetteConfigurazioneRegione l = new LinguetteConfigurazioneRegione (user, _funzione, "CMCapMenRelPage" , cdnLavoratore , "1", true , "LNDGNFNZ");
	...
	<af:form method="POST" action="AdapterHTTP" name="Frm1">
		...
		<input type="hidden" name="PAGE" value="CMCapMenRelPage" />		
		...
		<input type="hidden" name="strDescrGrado" value=""/>
<%
...
<%
			...
			if(readCodiciScaduti) {
				rows= serviceResponse.getAttributeAsVector("M_Load_Descr_Cap_MenRel_Scad.ROWS.ROW");
			} else {
				rows= serviceResponse.getAttributeAsVector("M_Load_Descr_Cap_MenRel.ROWS.ROW");
			}
%>
...
%>	
				<td width="75%" class="etichetta"><%=descrizioneCap%></td>
		    		<%String indice = "" + i;
