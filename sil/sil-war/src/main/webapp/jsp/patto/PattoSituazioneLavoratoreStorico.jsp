...
<% 		
	...
	SourceBean moduleBean = (SourceBean)serviceResponse.getAttribute("SITUAZIONELAVORATORESTORICO");
    //
    ...
    // non necessario nello storico if (notePatto==null) notePatto = new SourceBean("EMPTY");
    String strNote = Utils.notNull(notePatto.getAttribute("strnotesituazioneamm"));
    String prgPattoLavoratore = Utils.notNull(notePatto.getAttribute("prgPattoLavoratore"));
    BigDecimal klo = (BigDecimal)notePatto.getAttribute("numklopattolavoratore");
    String numklopattolavoratore = "";
    if (klo!=null) numklopattolavoratore = String.valueOf((klo).intValue()+1);
    ...
    ////////////////////////
    PageProperties pageProperties = new PageProperties("PattoCondizioneOccupazionalePage",null);
    PageAttribs pageAttsPattoAperto = new PageAttribs(user, "PattoCondizioneOccupazionalePage");
...
<pt:sections pageAttribs="<%= pageAttsPattoAperto%>">
    <pt:dynamicSection name="AM_OBBFO" titolo="Notizie sul diritto/dovere di istruzione e formazione" rows="<%= row_obbligoForma %>">
        ...
        <pt:sectionBody>
        <%            
        String obblForma_flg      = (String) row.getAttribute("FLGOBBLIGOFORMATIVO");
        String obblScolastico_flg = (String) row.getAttribute("FLGOBBLIGOSCOLASTICO");
        String descMod            = (String) row.getAttribute("DESCRIZIONE");
        %>
        <table width="100%">
            <tr>
                ...
                <% if (false) {
                    ...
                    <a  href="#" onclick="cancella('<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>','AM_OBBFO')"><img src="../../img/patto_elim.gif" ></a>
                    <%}%>
                </td>
            </tr>
            <tr>
    ...
    <pt:dynamicSection name="AM_EX_PS" titolo="Notizie sui cittadini stranieri" rows="<%= permSoggiornoRows %>">  
        ...
        <table width="100%">
            <tr>
              ...
              <%
                String codStatus = Utils.notNull(row.getAttribute("codStatus"));
              %>
              <td><b><%= (codStatus.equals("2")||codStatus.equals("5"))?"Carta di soggiorno":"Documento extracomunitario"%></b></td>
              
              <td class="etichetta2"><%= (codStatus.equals("2")||codStatus.equals("5"))?"Revoca carta di soggiorno":"Scadenza documento"%></td>
              <td><b><%=Utils.notNull(row.getAttribute("DATSCADENZA"))%>&nbsp;</b></td>
              <td class='pulsante_riga'>
                <% if (false && Utils.notNull(row.getAttribute("prgpermsogg")).length()>0) {
                    // valorizzo la sequenza di argomenti per la funzione js che cancella tutte le righe
                        args.append(Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA")));
                        args.append(",");                        
                    %>
                    <script>allArgs+="<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>,";</script>
                    <a  href="#" onclick="cancella('<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>','AM_EX_PS')"><img src="../../img/patto_elim.gif" ></a>
                    <%}%>
                </td>
            </tr>
            <tr>
              <td class="etichetta">Data richiesta/Sanatoria</td>
              <td colspan="3"><b><%=Utils.notNull(row.getAttribute("DATRICHIESTA"))%></b></td><td></td>
            </tr>
            <tr>
              <td class="etichetta">Stato richiesta</td>
              <td colspan="3"><b><%=Utils.notNull(row.getAttribute("DESCRIZIONERICH"))%></b></td><td></td>
            </tr>
            <tr>
              <td class="etichetta">Motivo richiesta</td>
              <td colspan="3"><b><%=Utils.notNull(row.getAttribute("DESCRIZIONEMOT"))%></b></td><td></td>
            </tr>
        </table>
    </pt:sectionBody>
	</pt:dynamicSection>
    <pt:dynamicSection name="AM_CM_IS" titolo="Liste speciali: collocamento mirato" rows="<%= collMiratoRows %>">
		...
		<table width="100%">
			<tr>
                ...
                <td class='pulsante_riga'>
                <% if (false && Utils.notNull(row.getAttribute("PRGCMISCR")).length()>0) {
                    // valorizzo la sequenza di argomenti per la funzione js che cancella tutte le righe
                        args.append(Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA")));
                        args.append(",");                        
                    %>
                    <script>allArgs+="<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>,";</script>
                    <a  href="#" onclick="cancella('<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>','AM_CM_IS')"><img src="../../img/patto_elim.gif" ></a>
                    <%}%>
                </td>
			</tr>
			<tr>
			   <td class="etichetta" >Categoria </td>
			   <td><b><%=Utils.notNull(row.getAttribute("DESCRIZIONEISCR"))%></b></td>
			...
			<tr>
			   ...
			   <td><b><%=Utils.notNull(row.getAttribute("DESCRIZIONEINV"))%></b></td>
			   <td></td><td></td>
               <td></td>
			</tr>
            <tr>
			   <td class="etichetta" >Percentuale invalidità</td>
			   <td><b><%=Utils.notNull(row.getAttribute("NUMPERCINVALIDITA"))%></b>&nbsp;%</td>
               <td></td><td></td><td></td>
			</tr>
		</table>
    	</pt:sectionBody>
    ...
    <pt:dynamicSection name="AM_MB_IS" titolo="Liste speciali: mobilità" rows="<%= mobilitaRows %>">
		...
		<table  width="100%">
			<tr>
			   ...
			   <td  class='pulsante_riga'>
                <% if (false && Utils.notNull(row.getAttribute("prgmobilitaiscr")).length()>0) {
                    // valorizzo la sequenza di argomenti per la funzione js che cancella tutte le righe
                        args.append(Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA")));
                        args.append(",");                        
                    %>
                    <script>allArgs+="<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>,";</script>
                    <a  href="#" onclick="cancella('<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>','AM_MB_IS')"><img src="../../img/patto_elim.gif" ></a>
                    <%}%>
                </td>
			</tr>			
           <tr>
			...
			<tr>
			   ...
			   <td colspan="3"><b><%=Utils.notNull(row.getAttribute("FLGINDENNITA"))%></b></td><td></td>
			</tr>
		</table>
	</pt:sectionBody>
    </pt:dynamicSection>
                  ...
                  <td colspan="3">
                   <table width="100%" border="0" cellpadding="0" cellspacing="0" >
                      <tr>
                        ...
                        <td class='pulsante_riga'>
                        <% if (false && Utils.notNull(row.getAttribute("PRGINDISPTEMP")).length()>0) {
                        // valorizzo la sequenza di argomenti per la funzione js che cancella tutte le righe
                            args.append(Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA")));
                            args.append(",");
                        %>
                        <script>allArgs+="<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>,";</script>
                        <a  href="#" onclick="cancella('<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>','AM_IND_T')"><img src="../../img/patto_elim.gif" ></a>
                        <%}%>
                        </td>
                      </tr>      
                   </table>
     ...
     <pt:dynamicSection name="PR_ESP_L" titolo="Ultime esperienze professionali: precedenti o in corso" rows="<%= movimentiRows %>">
        ...
        <table width="100%">
            <tr>
              ...
              <td class='pulsante_riga'>
                <% if (false && esp.getPrgEspLavoro().length()>0) {
                        // valorizzo la sequenza di argomenti per la funzione js che cancella tutte le righe
                            args.append(esp.getPrgLavPattoScelta());
                            args.append(",");
                %>
                    <script>allArgs+="<%=esp.getPrgLavPattoScelta()%>,";</script>
                     <a  href="#" onclick="cancella('<%=esp.getPrgLavPattoScelta()%>','PR_ESP_L')"><img src="../../img/patto_elim.gif" ></a>
                <%}%>             
              </td>
            </tr>
            <tr>
...
%>
			<tr>
              ...
              <td class='pulsante_riga'>
                <% if (false && tit.getPrgStudio().length()>0) {
                        // valorizzo la sequenza di argomenti per la funzione js che cancella tutte le righe
    ...
    <pt:dynamicSection name="PR_COR" titolo="Formazione professionale: precedente o in corso" rows="<%= formazProfRows %>">
        ...
        <table  width="100%">
            <tr>
                ...
                <td colspan="3"><b><%=Utils.notNull((String)row.getAttribute("CORSO"))%></b></td>
                <td class='pulsante_riga'>
                <% if (false && Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA")).length()>0) {
                    args.append(Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA")));
                    args.append(",");
                %>
                <script>allArgs+="<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>,";</script>
                <a  href="#" onclick="cancella('<%=Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"))%>','PR_COR')"><img src="../../img/patto_elim.gif" ></a>
                <%}%>
                </td>
            </tr>
            <tr>
                <td class="etichetta">Anno corso</td><td ><b><%=Utils.notNull((BigDecimal) row.getAttribute("NUMANNO"))%></b></td>
                <td class="etichetta">Percorso completato</td><td ><b><%=Utils.notNull((String)row.getAttribute("FLGCOMPLETATO"))%></b></td>
                <td></td>
            </tr> 
        </table>
        </pt:sectionBody>
        ...
        <td colspan=3>
            <table width="100%">
                <tr>
                    ...
                    <td class="campo"><af:textArea classNameBase="textarea" onKeyUp="fieldChanged();" name="STRNOTESITUAZIONEAMM" value="<%=strNote%>" cols="60" rows="5" readonly="<%=readOnlyStr%>"/></td>
                    <td>
    ...
    */
	private class EsperienzeProfWrapper {
        ...
        String getAnnoInizio() {
            return Utils.notNull((BigDecimal)row.getAttribute("NUMANNOINIZIO"));
        }
        String getAnnoFine() {
        ...
        String getMeseInizio() {
            return Utils.notNull((BigDecimal)row.getAttribute("NUMMESEINIZIO"));
        }
        String getRetribuzioneAnnua() {
            return Utils.notNull((BigDecimal)row.getAttribute("RETRIBANNUA"));
        }
        String getPrgLavPattoScelta() {return Utils.notNull(row.getAttribute("PRGLAVPATTOSCELTA"));}
        String getPrgPattoLavoratore(){return Utils.notNull(row.getAttribute("PRGPATTOLAVORATORE"));}
        String getPrgEspLavoro() {return Utils.notNull(row.getAttribute("PRGESPLAVORO"));}
		void setRow(SourceBean row) {this.row = row;}
	}
    /**/
    private class TitoloStudioWrapper {
        ...
        String getPrgStudio() {return Utils.notNull(row.getAttribute("PRGSTUDIO"));}
        void setRow(SourceBean row) {this.row = row;}
        String getPrgPattoLavoratore(){return Utils.notNull(row.getAttribute("PRGPATTOLAVORATORE"));}
    }
    ...
    private class StatoOccupazionaleWrapper {
        ...
        String getTipoAnzianita() {
            ...
            if(mesiAnz != null ){
                if(mesiAnz.compareTo(new BigDecimal(12))>0 || ( mesiAnz.compareTo(new BigDecimal(6)) > 0 && cod181!=null && cod181.equalsIgnoreCase("G") ))
        ...
        String getDataAnzianitaDisoccupazione() {
            return Utils.notNull((String)row.getAttribute("DATANZIANITADISOC"));
        }
        ...
        String getDataRicorsoGiudiziario(){
            return Utils.notNull((String)row.getAttribute("DATRICORSOGIURISDIZ"));    
        }
        String getStatoAtto(){
