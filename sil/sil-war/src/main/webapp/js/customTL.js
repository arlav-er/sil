...
function controllaFunzTL() {
	...
	return true;				// Torna dicendo che si pu\u00F2 fare la 'submit'
} //controllaFunzTL
 ...
 * E' importante perch\u00E8 abilita tutti i bottoni disabilitati dalla controllaFunzTL().
 */
function undoSubmit() {
...
/*Fa tornare il menu laterale completo se \u00E8 visualizzato quello specifico di un lavoratore*/
function rinfresca_laterale(){
	//renderizza il menu solo se ci\u00F2 \u00E8 necessario, ovvero \u00E8 specifico per il lavoratore ed io lo voglio generale
	var location_ = window.top.menu.location.toString();
	var index = location_.indexOf("CDNLAVORATORE");
	if (index != -1){		
...
/*
 * goConfirmGenericCustomTL(url, alertFlag)
 * E' la "goConfirm" generica usata in tutti i moduli di lista (quando 
 * l'utente clicca su un caption della lista).
 * NB: se la pagina \u00E8 gi\u00E0 in "submit" (blocco attivo) ritorna senza far nulla.
 * Viene usata dalle seguenti tags/classi:
 *  - it.eng.afExt.tags.DefaultListTag
 *  - it.eng.afExt.tags.JSButtonListTag
 *  - it.eng.sil.module.agenda.CalUtils
 *  - it.eng.sil.tags.AppuntamentiListTag
 */
 ...
 * In più, esegue la 'prepareSubmit()' per disabilitare ulteriori azioni sulla pagina.
 * Il parametro booleano facoltativo "doPrepare" indica se fare o meno la "prepareSubmit"
 * prima di trasferire il controllo alla nuova pagina (il valore di default \u00E8 TRUE).
 */
...
function isRequired(inputName) {
	...
	if (ctrlObj.value=="") {
		...
		if (ctrlObj.focus && !ctrlObj.disabled) {
			ctrlObj.focus();
		}
		return false;
	}
...
//controlla se il value di 'inputName' (nome di un textbox di tipo Number) \u00E8 un float con 'digitInt' cifre intere e 'digitDec' cifre decimali
function controllaFixedFloat(inputName, digitInt, digitDec) {
	var ctrlObj = eval("document.forms[0]." + inputName);
	var re = new RegExp(",","g");
  	...
  	if (ctrlObj.value == "")
    	...
    	if (s.length == 1) { 
 			if (eval(s[0].length > digitInt)) {
				alert("Numero nel campo " + ctrlObj.title + " pu\u00F2 essere al massimo di " + digitInt + " cifre intere e " + digitDec + " decimali");
			  	ctrlObj.focus();
			  	return false;
			} else {
    	...
    	if (s.length == 2) {
	      	if (eval(s[0].length > digitInt) || eval(s[1].length > digitDec)) {
		    	alert("Numero nel campo " + ctrlObj.title + " pu\u00F2 essere al massimo di " + digitInt + " cifre intere e " + digitDec + " decimali");
		      	ctrlObj.focus();
	          	return false;
	      	}
