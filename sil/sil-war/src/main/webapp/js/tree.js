...
var SPAZIO =     "<img src='" + IMG_BLANK + "' width=12 height=9>";
...
//-----------------------------------------------------------------
jsTree.prototype.createRoot = function(strLivello, strText, strURL, strTarget, closeAction) {

    this.root = new jsTreeNode(strLivello, strText, strURL, strTarget, closeAction);
    this.root.id = "root";
...
//  strTarget (string) - the target for the URL (optional).
//-----------------------------------------------------------------
function jsTreeNode(strLivello, strText, strURL, strTarget, closeAction, strFunzMenu) {
...
//  The jsTreeNode that was created.
//-----------------------------------------------------------------
jsTreeNode.prototype.addChild = function (strLivello, strText, strURL, strTarget, closeAction, strFunzMenu) {

    //create a new node (NCZ, 1/27/02)
    var objNode = new jsTreeNode(strLivello, strText, strURL, strTarget, closeAction, strFunzMenu );
...
jsTreeNode.prototype.addToDOM = function (objDOMParent) {
    ...
    d.writeln("<table cellspacing=\"0\" border=\"0\" class=\"menu\" id=\"tab_" + this.id +  "\"><tr>");
    ...
    if (this.livello == "1"){
        d.write("<td class=menu width='100%'><a class=menu "); 
		...
		// GG 22/12/2004 - aggiunto "return false" agli onClick.
        if (this.closeAction == true){
              d.write( "</td><td class=menu align='center'>");
              d.write( "<a href='#' onClick='chiudiMenuContest();return false'>");
              d.write( "<img src='../../img/chiudi_menu.gif' alt='Chiudi menu contestuale\r\n" + this.text + "'></a>");
        }
        ...
        if (this.childNodes.length > 0) {
        	...
        	d.write( "<a href='#' onClick=\"creaLink('" + this.id + "','"+ this.url + "');return false\">"); 
            d.write( SPAZIO_DOC );
            d.write( this.text  + "</a>");
        }
...
jsTree.prototype.espandiMenu = function(idMenu) {
...
jsTree.prototype.collassaMenu = function(idMenu) {

    //call method to add root to document, which will recursively
    //add all other nodes (NCZ, 1/27/02)
    var nodoLav = objLocalTree.nodes[idMenu];
    
    this.collassaAll(nodoLav);
}


jsTree.prototype.collassaAll = function(nodo) {

    if (nodo.childNodes.length>0){
        nodo.collapse()

        for (var i=0; i<nodo.childNodes.length; i++){
            this.collassaAll(nodo.childNodes[i])            
        }
    }    
	...
	jsTree.prototype.getText = function(nodo) {
		var nodoOb = objLocalTree.nodes[nodo];
		return nodoOb.text;    
	}

	
	jsTree.prototype.getText2 = function(myFunzMenu) {
	
		for ( i in objLocalTree.nodes){
			if (objLocalTree.nodes[i].funzMenu == myFunzMenu)	
				return objLocalTree.nodes[i].id;
		}
		return "";
