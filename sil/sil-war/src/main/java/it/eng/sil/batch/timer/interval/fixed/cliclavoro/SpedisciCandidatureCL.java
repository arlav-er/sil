...
import it.eng.sil.util.batch.BatchInvioCandidature;
 ...
 */
@Singleton
public class SpedisciCandidatureCL extends FixedTimerBatch {

	private Logger logger = Logger.getLogger(SpedisciCandidatureCL.class.getName());
	...
	public void perform() {
		if (this.isEnabled()) {
			...
			str[0] = "363";
			BatchInvioCandidature.main(str);
			if (logger.isDebugEnabled()) {
			...
			logger.warn(
					"[SpedisciCandidatureCL] ---> WARN: non abilitato su questo nodo, probabilmente è abilitato su un altro nodo");
		}
	}
}
