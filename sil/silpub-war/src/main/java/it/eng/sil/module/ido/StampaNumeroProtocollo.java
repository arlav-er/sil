 ...
 * commenti
 */
public class StampaNumeroProtocollo extends AbstractSimpleModule {
	...
	public void service(SourceBean request, SourceBean response) throws Exception {
		...
		String datChiam = StringUtils.getAttributeStrNotNull(request, "DATCHIAMATA");
		...
		if (!numProt.equals("")) {
			// Tokenizer per reperire il cpi e la data chiamata da usare nella
			// query se il punto di arrivo non è
			// la griglia interna delle stampe, caso in cui i dati necessari
			// sono passati singolarmente nella request
				...
				if (st.hasMoreTokens()) {
					...
					for (int j = 0; j < st2.countTokens(); j++) {
						if (st2.hasMoreTokens()) {
							...
							request.updAttribute("DATCHIAMATA", datChiam);
							// eseguo la query, la risposta viene inserita in un
							// SourceBean vuoto, sarà riempita successivamente
