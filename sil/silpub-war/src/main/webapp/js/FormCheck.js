//--------------------------------------------------------------
//NB: le funzioni che iniziano con '*' sono state aggiunte
//--------------------------------------------------------------
//
// FormChek.js
//
...
// IsPartitaIva(piva)				   True if checkDigit is correct
// IsCodFiscale(codfis)				   True if Codice Fiscale is correct
...
//
// prompt (s)                          (GG 10-4-2003 - RINOMINATA da "prompt" a "promptStatus" altrimenti sovrascrive la "prompt" della WINDOW"!!!)
// promptStatus (s)                    Display prompt string s in status bar.
...
//*checkFormatDate(objName)            Check that objName.value is a valid date, and reformat the value
...
// MODIFIED by Luigi Antenucci in 2003:
//  - sostituite "parseInt(x)" con "parseInt(x,10) per evitare conversioni in ottale! ;)
...
function stripInitialFinalWhitespace (s)

{ //posizione primo carattere dall'inizio diverso da blank
    var i = 0;
    while ((i < s.length) && charInString (s.charAt(i), whitespace))
       i++;
    ...
    var f = s.length-1;
    while ((f > i) && charInString (s.charAt(f), whitespace))
       f--;
...
// GG 25/03/2002 - funzione di TRIM di un oggetto che ha un "value" che è una stringa
// (ossia esegue il trim di un TextField dato come parametro)
function trimTextObject (lineTextObject) {
	var i, j;
	var stringa = lineTextObject.value;
	var l = stringa.length;
	i=0;
	while ( (i < l) && (stringa.charAt(i)==' ') )
		i++;

	if (i==l)
	
		lineTextObject.value = "";
		
	else {
	
		j=l-1;
		while ( (j >= 0) && (stringa.charAt(j)==' ') )
			j--;
		lineTextObject.value = stringa.substring(i, j+1);
...
/*  ================================================================
    ...
    INPUT:     objName - an INPUT object conteining a date

    RETURNS:  true, if the object values is a valid Diner's
	      ==> it also reformatting the data to
	          DD/MM/YYYY format
...
function IsPartitaIva(piva)
{
	...
	for (i = 0; i < piva.length-1; i++)
   	{
   		var c = parseInt(piva.charAt(i),10);
   	...
   	//alert("sumDispari: " + sumDispari);
   	//alert("sumPari1: " + sumPari1);
   	//alert("sumPari2: " + sumPari2);

   	var Totale = new String(sumDispari + sumPari1 + sumPari2);
   	//alert("Totale: " + Totale);
   	var tmpCheck = Totale.charAt(Totale.length - 1);
   	//alert("tmpCheck: " + tmpCheck);

   	var CheckDigit = 0;
   	if (tmpCheck >= 1 && tmpCheck <= 9)
   		CheckDigit = 10 - tmpCheck;
   	else
   		CheckDigit = 0;
	...
	if (CheckDigit == piva.charAt(piva.length-1))
	{
		//alert("PIVA OK ");
  		return true;
  	}
...
function IsCodFiscale(codfis)
{
	...
	for (k=0; k<codfis.length; k++)
	{
		var car = codfis.charAt(k);

		if (k==6 || k==7 || k==9 || k==10 /* || k==12 || k==13 || k==14 */) 	//controllo caratteri numerici 
		{
			//M.F. 30/09/2002 N.B. gli ultimi 3 caratteri possono essere anche alfabetici in caso di omonimie
			if (!isDigit(car)) {
			...
			if (k==0 || k==1 || k==2 || k==3 || k==4 || k==5 ){ 	//controllo caratteri alfabetici
			//M.F. 30/09/2002 N.B. solo i primi sei caratteri devono essere alfabetici
				if (isDigit(car)){
					return false;
				} 
			}
	...
	tabCarPari = new Array(36)
	for (i=0; i < tabCarPari.length; i++)
	{
		tabCarPari[i] = new Array(2)
	}
	...
	tabCarDispari = new Array(36)
	for (i=0; i < tabCarDispari.length; i++)
	{
		tabCarDispari[i] = new Array(2)
	}
	...
	tabCarCheck = new Array(26)
	for (i=0; i < tabCarCheck.length; i++)
	{
		tabCarCheck[i] = new Array(2)
	}
	...
	var numPari = 0;		//sommatoria caratteri pari
	var numDispari = 0;		//sommatoria caratteri dispari
	for (i = 0; i < codfis.length-1; i++)
   	{
   		var c = codfis.charAt(i);
   		//alert("c: -" + c + "-");
   		if ((i+1)%2 != 0)  //numeri dispari
   		{
   			//alert("dispari");
   			var tmpNum = 0;
   			for(j=0; j<tabCarDispari.length; j++)
			{
				 if (tabCarDispari[j][0] == c)
				 {
				 	tmpNum = parseInt(tabCarDispari[j][1],10);
				 	break;
				 }
			}
   			numDispari += tmpNum;
   		...
   		else			// numeri pari
   		{
   			...
   			var tmpNum = 0;
   			for(j=0;  j<tabCarPari.length; j++)
			{
				 if (tabCarPari[j][0] == c)
				 {
				 	tmpNum = parseInt(tabCarPari[j][1],10);
				 	break;
				 }
			}
   			numPari += tmpNum;
   	...
   	var chekDigit = (numPari + numDispari)%26;


   	var tmpCheck = codfis.charAt(codfis.length-1);
   	var tmpCheckInt = 0;


	for(j=0;  j<tabCarCheck.length; j++)
	{
		 if (tabCarCheck[j][0] == tmpCheck)
		 {
		 	tmpCheckInt = parseInt(tabCarCheck[j][1],10);
		 	break;
		 }
	}

 	if (chekDigit == tmpCheckInt)
	{
		//alert("COD-FISCALE OK ");
  		return true;
  	}
  	else
  	{
  		//alert("COD-FISCALE KO ");
  		return false;
  	}
...
function checkFormatTime( obj )
{
  ...
  if(valore == "") { return true; }
	...
	var ore = parseInt(pezzi[0], 10);
	var minuti = parseInt(pezzi[1], 10);
...
// FV 18/11/2003 9.54
// Funzione che verifica se il campo passato come parametro ha raggiunto la lunghezza 
// massima di caratteri inputabili. Questa viene passata come parametro
 function CheckLen(len, inputName){
  var campo = eval("document.forms[0]." + inputName);
  
  var stringa = new String(campo.value);
  if (stringa.length > len)
  {
   alert ("Lunghezza massima ["+ len + " caratteri] del campo " + campo.title + " raggiunta");
