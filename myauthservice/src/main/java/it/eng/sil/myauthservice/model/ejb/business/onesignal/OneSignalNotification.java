...
public class OneSignalNotification extends ProviderNotification {
	...
	private it.eng.sil.myauthservice.model.ConstantsSingleton constantSingleton;

	public OneSignalNotification(String projectId, ConstantsSingleton constants) {
		this.providerId = ProviderNotification.ONE_SIGNAL_PROVIDER_ID;
		this.projectId = projectId;
		this.constantSingleton = constants;
	...
	private void setProviderConfiguration() {
		...
		case ProviderNotification.LAVORO_PER_TE_PROJECT_ID:
			this.appId = constantSingleton.getOneSignalAppId();
			this.authorizationKey = constantSingleton.getOneSignalAuthorization();
			this.destinationUrl = constantSingleton.getOneSignalNotificationEndpoint();
			break;
		default:
			break;
		}
	...
	private HttpURLConnection createConnection(String urlParams, String requestedMethod)
		...
		try {
			...
			/*if (!constantSingleton.getProxyHostSms().isEmpty()) {
				  final java.net.Proxy PROXY = new java.net.Proxy(java.net.Proxy.Type.HTTP,
						new InetSocketAddress(constantSingleton.getProxyHostSms(), constantSingleton.getProxyPortSms()));
	...
	private String formatResponseSend(JSONObject jsonObject) throws ProviderNotificationException {
		...
		if (jsonObject != null) {
			try {
				...
				throw new OneSignalException(
						"JSONException durante la chiamata OneSignal - " + e.getMessage());
			}
		} else {
