package it.eng.myportal.beans.utente.messaging;
...
@ManagedBean(name = "utenteMessagingBean")
@ViewScoped
public class UtenteMessagingBean extends AbstractBaseBean {
	...
	public void initRequestSupport() {
		log.info("initRequestSupport called");
		// implement equal hashcode in cvDatiPersonali e cvLetteraAcc
		cvDatiPersonaliList = utenteInfoHome.getAllCurriculaNoCopie(pfPrincipalId);
		cvLetteraAccList = utenteInfoHome.getAllLettereAccNoCopie(pfPrincipalId);
	}
	...
	public void sendRichiesta(String tipoMessaggio) {
		...
		if (ConstantsSingleton.MsgMessaggio.ESPERTO.equalsIgnoreCase(tipoMessaggio)) {
			...
			msgMessaggioEJB.saveMsgSupporto(message, pfPrincipalId);
		}
	}
	...
	public void loadMessageContattiInviati() {
		try {
			...
			log.error("Errore durante la ricerca del messaggio mediante ticket in loadMessageContattiInviati(): " + ex.getMessage());
			RequestContext.getCurrentInstance().addCallbackParam("notFound", true);
		}
	...
	public void loadMessageContattiRicevuti() {
		try {
			...
			log.error("Errore durante la ricerca del messaggio mediante ticket in loadMessageContattiRicevuti(): " + ex.getMessage());
			RequestContext.getCurrentInstance().addCallbackParam("notFound", true);
		}
