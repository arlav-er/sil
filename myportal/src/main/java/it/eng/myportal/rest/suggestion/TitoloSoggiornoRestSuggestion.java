 ...
 */
@Stateless
@Path("rest/titoloSoggiorno/")
public class TitoloSoggiornoRestSuggestion extends AbstractAlberoRestSuggestion<DeTitoloSoggiornoDTO>  {
	
	@EJB
	transient DeTitoloSoggiornoHome deTitoloSoggiornoHome;

	@Override
	protected ITreeableHome<DeTitoloSoggiornoDTO> getHome() {
		return deTitoloSoggiornoHome;
	}
	...
	public String albero(@QueryParam("key") String par) {
		...
		List<DeTitoloSoggiornoDTO> list;
		try {
			if (par == null) {
				par = "0";
			}
			
			list = deTitoloSoggiornoHome.findByCodPadre(par);
