...
@Stateless
@Path("rest/mansioni/")
public class MansioniRestSuggestion extends AbstractAlberoRestSuggestion<DeMansioneDTO> {

	@EJB
	transient DeMansioneHome deMansioneHome;

	@Override
	protected ITreeableHome<DeMansioneDTO> getHome() {
		return deMansioneHome;
	}
	...
	public String albero(@QueryParam("key") String par) {
		...
		List<DeMansioneDTO> list;
		try {
			if (par == null) {
				par = "0";
			}

			list = deMansioneHome.findByCodPadre(par);
			...
			if (!isResEmpty) {
				...
				for (DeMansioneDTO element : list) {
