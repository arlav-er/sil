...
/**
 * Servlet per l'autocomplete delle lingue.
...
@Stateless
@Path("rest/lingua/")
public class LinguaRestSuggestion extends AbstractAlberoRestSuggestion<DeLinguaDTO>  {
	
	@EJB
	transient DeLinguaHome deLinguaHome;

	@Override
	protected ITreeableHome<DeLinguaDTO> getHome() {
		return deLinguaHome;
	}
	...
	public String albero(@QueryParam("key") String par) {
		...
		List<DeLinguaDTO> list;
		try {
			if (par == null) {
				par = "0";
			}
			
			list = deLinguaHome.findByCodPadre(par);
