...
@FacesConverter("deAttivitaMinConverter")
public class DeAttivitaMinConverter implements Converter {
		...
		if (value.trim().equals("")) {
			...
			try {
				...
				deAttivitaMinHome = (DeAttivitaMinHome) ic
						.lookup("java:module/DeAttivitaMinHome");
			} catch (NamingException e1) {
				return null;
			}
			DeAttivitaMinDTO deAttivitaMinDTO = deAttivitaMinHome
					.findDTOById(value);
