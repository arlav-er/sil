...
public class MyPortalHttpLoader implements HttpLoader {
	
	/**
	 * Inizializzo il proxy staticamente in modo che venga creato una volta per tutte e via.
	 */
	...
	public String httpGet(String urlS) {
		...
		try {
			...
			if (ConstantsSingleton.Proxy.ACTIVE)
				connection = url.openConnection(ConstantsSingleton.Proxy.PROXY);
			else
