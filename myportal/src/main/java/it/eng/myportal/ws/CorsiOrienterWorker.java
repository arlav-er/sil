...
import it.eng.myportal.entity.home.OrEdizioneAvviataHome;
import it.eng.myportal.entity.home.OrEnteCorsoHome;
import it.eng.myportal.entity.home.OrSedeCorsoHome;
...
@Stateless
public class CorsiOrienterWorker {

	private static final String QUALIFICA_SRQ_NULL = "0000";

	// private static final String _04_INPUT_XML_NON_VALIDO =
	// "InputXML non valido";
	...
	public void processNodoCorso(Node nCorso, XPath xpath) throws XPathExpressionException, ParseException,
		...
		if (isInsUpd) {
			...
			insertUpdateCorso(nCorso, xpath, idCorso, oc);
	...
	private void insertUpdateCorso(Node nCorso, XPath xpath, Integer idCorso, OrCorsoDTO oc)
			throws XPathExpressionException, ParseException, CorsoNonAllineatoException {
		...
		log.info("orEdizioneAvviatas persist size:" + (orEdizioneAvviatas != null ? orEdizioneAvviatas.size() : 0));
		...
		log.info("orSedeCorsos persist size:" + (orSedeCorsos != null ? orSedeCorsos.size() : 0));
	...
	private Integer persistCorsoAndCheck(Integer idCorso, OrCorsoDTO oc, OrEnteCorsoDTO ec)
		...
		boolean trovatoComune = StringUtils.equals(ecPersist.getCodComune(), StringUtils.trimToNull(ec.getCodComune()));
		if (!trovatoComune) {
			log.warn("codComune xml:" + ec.getCodComune() + ", codComune risolta:" + ecPersist.getCodComune());
		}
		boolean trovatoTipoFormazione = StringUtils.equals(ocPersist.getCodTipoFormazione(),
				StringUtils.trimToNull(oc.getCodTipoFormazione()));
		// codQualificaSRQ non obbligatorio, cmq OK il codice inesistente
		// 0000
		if (!trovatoTipoFormazione) {
			log.warn("codTipoFormazione xml:" + oc.getCodTipoFormazione() + ", codTipoFormazione risolta:"
					+ ocPersist.getCodTipoFormazione());
		}
		boolean trovataQualificaSrq = QUALIFICA_SRQ_NULL.equalsIgnoreCase(oc.getCodQualificaSrqRilasciata())
				|| StringUtils.equals(ocPersist.getCodQualificaSrqRilasciata(),
						StringUtils.trimToNull(oc.getCodQualificaSrqRilasciata()));
		if (!trovataQualificaSrq) {
			log.warn("CodQualificaSrq xml:" + oc.getCodQualificaSrqRilasciata() + ", CodQualificaSrq risolta:"
					+ ocPersist.getCodQualificaSrqRilasciata());
		}
		boolean trovataProfessione = StringUtils.equals(ocPersist.getCodProfessione(),
				StringUtils.trimToNull(oc.getCodProfessione()));
		if (!trovataProfessione) {
			log.warn("codProfessione xml:" + oc.getCodProfessione() + ", codProfessione risolta:"
					+ ocPersist.getCodProfessione());
		}
		boolean trovataMansione = StringUtils.equals(ocPersist.getCodMansione(),
				StringUtils.trimToNull(oc.getCodMansione()));
		if (!trovataMansione) {
