...
if (!myportal.modalDialog) {
	myportal.modalDialog = {
			init: function(options) {
				...
				myDiv.open = function(openP,elem) {
					...
					//InpuText modale
					if($(jq(options.ccid + "\:modal_form\:descrizione_copia")).length){
						$(jq(options.ccid + "\:modal_form\:descrizione_copia"))[0].focus();
						$(jq(options.ccid + "\:modal_form\:descrizione_copia"))[0].setSelectionRange(0, $(jq(options.ccid + "\:modal_form\:descrizione_copia"))[0].value.length);
					}
					return false;
				};
